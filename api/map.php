...
$key = $input->get('api_key') ? safe_replace($input->get('api_key')) : $setting['api_key'];
$key = str_replace(array('/','(',')','&',';'),'',$key);
$maptype = $input->get('maptype') ? intval($input->get('maptype')) : ($setting['maptype'] ? $setting['maptype'] : 1);
$defaultcity = $input->get('defaultcity') ? $input->get('defaultcity') : ($setting['defaultcity'] ? $setting['defaultcity'] : '北京');
...
function addMarker(){       
    ...
    $('#<?php echo $field?>').val(center.lngX+'|'+center.latY+'|'+ZoomLevel);
} 
...
function mouseUp(param){ 
    ...
    var ZoomLevel = mapObj.getZoomLevel();
    $('#<?php echo $field?>').val(lngX+'|'+latY+'|'+ZoomLevel);
}
...
<?php 
} elseif(!empty($input->get('city')) && $maptype==1) {
    if(!$input->get('city'))  showmessage(L('error'));
    $city = urldecode(trim($input->get('city')));
