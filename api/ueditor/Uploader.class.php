...
class Uploader
{
    ...
    public function __construct($fileField, $config, $type = "upload") {
        ...
        $this->config = $config;
        $this->type = $type;
        $this->siteid = intval($this->config['siteid']) ? intval($this->config['siteid']) : 1;
        ...
        if ($this->isadmin==0 && !$this->grouplist[$this->groupid]['allowattachment']) {
            // 错误提示
    ...
    public function getFileInfo()
    {
        return array(
            ...
            "original" => strstr($this->oriName, '.', true),
            "type" => $this->fileType,
            "size" => $this->fileSize
        );
    }
