...
if((!$input->get('callback') && $input->get('act') != 'ajax_select')|| !$input->get('act'))  showmessage(L('error'));
...
switch($input->get('act')) {
	...
	case 'ajax_getpath':
		ajax_getpath($input->get('parentid'),$input->get('keyid'),$input->get('callback'));
	break;	
	case 'ajax_gettopparent':
		ajax_gettopparent($input->get('linkageid'),$input->get('keyid'),$input->get('callback'));
	break;
	case 'ajax_select':
		$parent_id = $input->get('parent_id') ? intval($input->get('parent_id')) : 0;
...
function ajax_getlist() {
	$keyid = intval($input->get('keyid'));
	$datas = getcache($keyid,'linkage');
	$infos = $datas['data'];
	$where_id = $input->get('parentid') ? $input->get('parentid') : intval($infos[$input->get('linkageid')]['parentid']);
