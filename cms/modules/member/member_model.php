...
class member_model extends admin {
	...
	function add() {
		if(IS_POST) {
			...
			if($this->db->table_exists($info['tablename'])) dr_admin_msg(0,L('operation_failure'),'?m=member&c=member_model&a=manage', '', 'add');
	...
	function import(){
		if(IS_POST) {
			...
			if($this->db->table_exists($info['tablename'])) dr_admin_msg(0,L('operation_failure'),'?m=member&c=member_model&a=manage');
