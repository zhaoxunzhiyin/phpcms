...
class index extends foreground {
	...
	private $input,$cache,$email,$times_db,$favorite_db,$friend_db,$rid,$att_db,$verify_db,$smsapi,$siteinfo,$menu,$grouplist,$member_model;
	...
	public function register() {
		...
		if(IS_POST) {
			...
			if($userid > 0) {
				...
				//执行登录操作
				$cookietime = $member_setting['logintime'] ? max(intval($member_setting['logintime']), 500) : 86400;
	...
	public function public_get_password_type() {
		pc_base::load_sys_class('service')->assign('siteid', intval($this->input->get('siteid')));
