...
class index extends foreground {
	...
	public function logout() {
		...
		$forward = $this->input->get('forward') && trim($this->input->get('forward')) ? $this->input->get('forward') : (HTTP_REFERER ? dr_safe_url(HTTP_REFERER) : APP_PATH.'index.php?m=member&c=index&a=login');
		dr_msg(1, L('logout_success'), IS_AJAX ? ['url' => $forward] : $forward);
	}
