...
<form name="myform" id="myform" action="?m=member&c=membermodel&a=delete" method="post" onsubmit="check();return false;">
...
?>
    <tr>
        ...
        <td align="center"><input type="text" onblur="dr_ajax_save(this.value, '<?php echo '?m=member&c=membermodel&a=sort&modelid='.$v['modelid'].'&menuid='.$this->input->get('menuid');?>')" value="<?php echo $v['sort'];?>" class="displayorder form-control input-sm input-inline input-mini"></td>
        <td align="center"><?php echo $v['modelid']?></td>
        <td><?php echo $v['name']?> / <?php echo $v['tablename']?></td>
        <td class="table-center" style="text-align:center"><a href="javascript:;" onclick="dr_ajax_open_close(this, '<?php echo '?m=member&c=membermodel&a=public_disabled&modelid='.$v['modelid'].'&menuid='.$this->input->get('menuid');?>', 1);" class="badge badge-<?php echo $v['disabled'] ? 'no' : 'yes';?>"><i class="fa fa-<?php echo $v['disabled'] ? 'times' : 'check';?>"></i></a></td>
        <td align="center"><a class="btn btn-xs blue" href="javascript:dr_iframe_show('<?php echo L('field').L('manage');?>','?m=member&c=member_modelfield&a=manage&modelid=<?php echo $v['modelid']?>&menuid=<?php echo $this->input->get('menuid');?>&is_menu=1', '80%', '90%');"> <i class="fa fa-code"></i> <?php echo L('field').L('manage')?></a> <a class="btn btn-xs green" href="javascript:edit(<?php echo $v['modelid']?>, '<?php echo $v['name']?>')"> <i class="fa fa-edit"></i> <?php echo L('edit')?></a> <a class="btn btn-xs yellow" href="?m=member&c=membermodel&a=export&modelid=<?php echo $v['modelid']?>"> <i class="fa fa-sign-out"></i> <?php echo L('export')?></a> <a class="btn btn-xs dark" href="javascript:move(<?php echo $v['modelid']?>, '<?php echo $v['name']?>')"> <i class="fa fa-arrows"></i> <?php echo L('move')?></a></td>
    </tr>
<?php
    }
?>
...
function edit(id, name) {
    artdialog('edit','?m=member&c=membermodel&a=edit&modelid='+id,'<?php echo L('edit').L('member_model')?>《'+name+'》',700,500);
}
function move(id, name) {
    artdialog('move','?m=member&c=membermodel&a=move&modelid='+id,'<?php echo L('move')?>《'+name+'》',700,500);
}
function check() {
    if(myform.action == '?m=member&c=membermodel&a=delete') {
        var ids='';
