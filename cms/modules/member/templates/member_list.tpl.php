...
?>
    <tr>
		...
		<?php }}?>
		<td>
			<label><a href="javascript:edit(<?php echo $v['userid']?>, '<?php echo $v['username']?>')" class="btn btn-xs green"><?php echo L('edit')?></a></label>
			<label><a href="?m=member&c=member&a=public_alogin&id=<?php echo $v['userid']?>" target="_blank" class="btn btn-xs red"><?php echo L('login')?></a></label>
            <?php foreach ($clink as $a) {
                if ($a['model'] && $a['check'] && method_exists($a['model'], $a['check']) && call_user_func(array($a['model'], $a['check']), ROUTE_M, $v) == 0) {
                    continue;
                }
