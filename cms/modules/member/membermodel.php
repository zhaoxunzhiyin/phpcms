...
class membermodel extends admin {
	...
	function add() {
		if(IS_POST) {
			...
			if($this->db->table_exists($info['tablename'])) dr_admin_msg(0,L('operation_failure'));
			...
			if($modelid) {
				...
				dr_admin_msg(0,L('operation_failue'));
			}
		} else {
	...
	function edit() {
		if(IS_POST) {
			$info = $this->input->post('info');
			$modelid = isset($info['modelid']) ? $info['modelid'] : dr_admin_msg(0,L('operation_success'));
			if(!$info['name']) dr_admin_msg(0,L('input').L('model_name'), array('field' => 'name'));
	...
	function import(){
		if(IS_POST) {
			...
			if($this->db->table_exists($info['tablename'])) dr_admin_msg(0,L('operation_failure'),'?m=member&c=membermodel&a=manage&menuid='.$this->input->get('menuid'));
			...
			if($modelid) {
				...
				//更新模型缓存
				$this->cache();
				dr_admin_msg(1,L('operation_success'),'?m=member&c=membermodel&a=manage&menuid='.$this->input->get('menuid'));
			}
		} else {
