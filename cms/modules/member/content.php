...
class content extends foreground {
	private $input,$cache_api,$priv_db,$content_db,$content_check_db,$model,$sitemodel_db,$siteinfo,$menu,$grouplist,$member_model;
	...
	public function delete(){
		...
		$array = $this->content_check_db->get_one($where);
		if(!$array){
			showmessage(L('operation_failure'), HTTP_REFERER); 
		}else{
			$this->content_db->set_model($modelid);
			$r = $this->content_db->get_one(array('id'=>$id));
			$this->content_db->delete(array('id'=>$id));
			$this->content_db->table_name = $this->content_db->table_name.'_data_'.$r['tableid'];
			$this->content_db->delete(array('id'=>$id));
			$this->content_db->set_model($modelid);
			$total = $this->content_db->count();
			$this->sitemodel_db = pc_base::load_model('sitemodel_model');
			$this->sitemodel_db->update(array('items'=>$total),array('modelid'=>$modelid));
			$this->content_check_db->delete(array('checkid'=>$checkid));//删除对应投稿表
