...
function replace_pc_tag($filepath, $old_tag, $new_tag, $style, $dir) {
	if (file_exists($filepath)) {
		...
		$old_tag && $data = str_replace($old_tag, $new_tag, $data);
...
function visualization($html, $style = '', $dir = '', $file = '') {
	$change = load_css(CSS_PATH.'admin_visualization.css').load_js(JS_PATH.'Dialog/main.js')."
		...
		".load_js(JS_PATH.'admin_common.js').load_js(CSS_PATH.'bootstrap/js/bootstrap.min.js')."
		<script type='text/javascript'>
		$(function(){
		...
		var w = 700;
		var h = 500;
		omnipotent('edit',url+'&style=$style&dir=$dir&file=$file&pc_hash='+pc_hash+'&'+$(this).parent('.admin_piao').attr('data'),'".L('pc_tag','' ,'template')."',0,w,h);})
		$('.admin_block').click(function(){
			omnipotent('edit','?m=block&c=block_admin&a=block_update&id='+$(this).attr('blockid')+'&pc_hash='+pc_hash,'".L('pc_tag','' ,'template')."',0,w,h);
		});
	})</script>";
