...
<?php if ($dir !='' && $dir != '.'):?>
<tr>
<td align="left" colspan="5"><a href="<?php echo '?m=template&c=file&a=init&style='.$this->style.'&dir='.trim(dirname($dir), '.');?>"><img src="<?php echo IMG_PATH?>folder-closed.png" /><?php echo L("parent_directory")?></a></td>
</tr>
<?php endif;?>
<?php 
if(is_array($list)):
...
<tr>
<?php if (is_dir($v)) {
    echo '<td align="left"><img src="'.IMG_PATH.'folder-closed.png" /> <a href="?m=template&c=file&a=init&style='.$this->style.'&dir='.($this->input->get('dir') && !empty($this->input->get('dir')) ? urlencode($this->input->get('dir').'/') : '').$filename.'"><b>'.$filename.'</b></a></td><td align="left"><label style="width: 100%;"><input type="text" name="file_explan['.$encode_local.']['.$filename.']" value="'.(isset($file_explan[$encode_local][$filename]) ? $file_explan[$encode_local][$filename] : "").'"></label></td><td style="text-align:center"> - </td><td>'.dr_date(filemtime($this->filepath.$dir.'/'.$filename), null, 'red').'</td><td></td>';
} else {
     if (substr($filename,-4,4) == 'html') {
        echo '<td align="left"><img src="'.IMG_PATH.'file.png" /> '.$filename.'</td><td align="left"><label style="width: 100%;"><input type="text" name="file_explan['.$encode_local.']['.$filename.']" value="'.(isset($file_explan[$encode_local][$filename]) ? $file_explan[$encode_local][$filename] : "").'"></label></td><td style="text-align:center">'.format_file_size(filesize($this->filepath.$dir.'/'.$filename)).'</td><td>'.dr_date(filemtime($this->filepath.$dir.'/'.$filename), null, 'red').'</td>';
        echo '<td>';
        if($tpl_edit=='1'){
