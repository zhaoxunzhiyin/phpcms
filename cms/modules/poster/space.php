...
class space extends admin {
	...
	public function add() {
		if (IS_POST) {
			...
			if(!$space['name']) dr_admin_msg(0,L('please_input_space_name'), array('field' => 'name'));
			if(!$space['type']) dr_admin_msg(0,L('choose_space_type'), array('field' => 'type'));
			if(dr_is_empty($space['width'])) dr_admin_msg(0,L('input_width_height'), array('field' => 'width'));
			if(dr_is_empty($space['height'])) dr_admin_msg(0,L('input_width_height'), array('field' => 'height'));
