...
class index extends admin {
	...
	public function sqlreplace() {
		if (IS_AJAX_POST) {
			...
			} elseif (dr_in_array($db_field, $this->db->get_primary($db_table))) {
				dr_json(0, implode('，', $this->db->get_primary($db_table)).L('主键不支持替换'));
			}
			...
			if ($this->input->post('replace_type') == 2) {
				...
				if ($ck_pr && !dr_in_array($this->input->post('db_pr_field'), $ck_pr)) {
					dr_json(0,L('select_is_fieldfield') . implode('，', $ck_pr) . L('pleasereselect'));
				}
