        ...
        <li class="dropdown"> <a href="javascript:;" class="on"> <i class="fa fa-users"></i>  <?php echo L('用户（总）');?></a> <a class="dropdown-toggle on" data-toggle="dropdown" data-hover="dropdown" data-close-others="true" aria-expanded="false"><i class="fa fa-angle-double-down"></i></a>
            ...
            if(is_array($infos)){
                foreach($infos as $info){
                    ...
                    echo "<li><a href=\"?m=content&c=content&a=initall&modelid=".$modelid."&menuid=".$param['menuid']."&start_time=&end_time=&posids=&field=username&keyword=".$info['username']."&pc_hash=".dr_get_csrf_token()."\" class=\"";
        ...
        <li class="dropdown"> <a href="javascript:;" class="on"> <i class="fa fa-user"></i>  <?php echo L('用户（今）');?></a> <a class="dropdown-toggle on" data-toggle="dropdown" data-hover="dropdown" data-close-others="true" aria-expanded="false"><i class="fa fa-angle-double-down"></i></a>
            ...
            if(is_array($infos)){
                foreach($infos as $info){
                    ...
                    echo "<li><a href=\"?m=content&c=content&a=initall&modelid=".$modelid."&menuid=".$param['menuid']."&start_time=".dr_date(SYS_TIME,'Y-m-d')."&end_time=".dr_date(SYS_TIME,'Y-m-d')."&posids=&field=username&keyword=".$info['username']."&pc_hash=".dr_get_csrf_token()."\" class=\"";
...
?>   
    ...
    <td align="center"><label><a href="<?php
        ...
        <?php foreach ($clink as $a) {
            if ($a['model'] && $a['check'] && method_exists($a['model'], $a['check']) && call_user_func(array($a['model'], $a['check']), ROUTE_M, $r) == 0) {
                continue;
            }
