...
<fieldset>
  ...
  <tr>
    <th><?php echo L('解析自定义URL')?>：</th>
    <td class="y-bg"><div class="mt-radio-inline">
        <label class="mt-radio mt-radio-outline"><input type="radio" name="setting[not_diy_url_route]" value="1"  /> <?php echo L('手动解析')?> <span></span></label>
        <label class="mt-radio mt-radio-outline"><input type="radio" name="setting[not_diy_url_route]" value="0" checked /> <?php echo L('自动解析')?> <span></span></label>
    </div>
    <div class="onShow"><?php echo L('自动解析不需要配置rewrite.php规则但会产生缓存文件；手动解析需要配置rewrite.php规则，大数据推荐')?></div></td>
  </tr>
  <tr>
