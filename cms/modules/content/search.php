...
class search {
	...
	public function init() {
		...
		if($info && $module_search) {
			//搜索间隔
			if (isset($this->module['setting']['search']['search_time'])
			...
			&& $params['page'] <= 1) {
				...
				if (!$ctime) {
					...
					dr_msg(0, L('search_minrefreshtime',array('min'=>$this->module['setting']['search']['search_time'])), 'index.php?m=content&c=search&catid='.$catid);
				} else {
					$this->cache->set_auth_data($cname, SYS_TIME);
				}
			}
			...
			foreach ($fields as $field=>$r) {
				if($r['issystem'] && $r['issearch']) {
					if($r['formtype']=='catid') {
						if($info['catid']) $where .= " " . (intval($this->module['setting']['search']['is_double_like']) ? "OR" : "AND") . " ".(isset($this->module['setting']['search']['search_catid']) && $this->module['setting']['search']['search_catid'] ? "catid in (".($cat['arrchildid'] ? $cat['arrchildid'] : (isset($info['catid']) ? $info['catid'] : $catid)).")" : "catid='".(isset($info['catid']) ? $info['catid'] : $catid)."'");
					} elseif($r['formtype']=='number') {
						$start = "{$field}_start";
						$end = "{$field}_end";
						if($params[$start]) {
