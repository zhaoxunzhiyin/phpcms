...
class search {
	...
	public function init() {
		...
		if (is_mobile($siteid) && $sitelist[$siteid]['mobileauto'] || defined('IS_MOBILE') && IS_MOBILE) {
			if (!file_exists(PC_PATH.'templates'.DIRECTORY_SEPARATOR.$default_style.DIRECTORY_SEPARATOR.'mobile'.DIRECTORY_SEPARATOR.'search.html')) {
				define('ISMOBILE', 0);
				define('IS_HTML', 0);
				include template('content','search',$default_style);
			} else {
