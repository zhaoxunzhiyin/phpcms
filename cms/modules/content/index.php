...
class index {
	...
	public function lists() {
		...
		pc_base::load_app_class('url', 'content')->category_url($catid, $page);
		pc_base::load_app_class('url', 'content')->category_url($catid, '[page]');
		...
		if($type==0) {
			...
			$GLOBALS['URL_ARRAY']['categorydir'] = get_categorydir($catid);
			$GLOBALS['URL_ARRAY']['parentdir'] = get_parentdir($catid);
	...
	public function show() {
		...
		if (!$data) {
			...
			foreach ($this->form_cache['field'] as $t) {
				if ($t['formtype'] == 'redirect' && $rs[$t['field']]) {
					// 存在转向字段时的情况
					...
					pc_base::load_sys_class('service')->assign('goto_url', $rs[$t['field']]);
		...
		$data['id'] = $rs['id'];
		$data['style'] = $rs['style'];
		$data['tableid'] = $rs['tableid'];
		$data['sysadd'] = $rs['sysadd'];
		$data['paginationtype'] = $rs['paginationtype'];
		$data['maxcharperpage'] = $rs['maxcharperpage'];
		...
		//上一页
		$data['previous_page'] = $this->db->get_one(array('catid' => (int)$catid, 'id<' => (int)$id, 'status' => 99), '*', 'id DESC');
		if(isset($this->form_cache['setting']['previous']) && $this->form_cache['setting']['previous'] && !$data['previous_page']) {
			$data['previous_page'] = $this->db->get_one(array('catid' => (int)$catid, 'status' => 99), '*', 'id DESC');
		}
		//下一页
		$data['next_page'] = $this->db->get_one(array('catid' => (int)$catid, 'id>' => (int)$id, 'status' => 99), '*', 'id ASC');
		if(isset($this->form_cache['setting']['previous']) && $this->form_cache['setting']['previous'] && !$data['next_page']) {
			$data['next_page'] = $this->db->get_one(array('catid' => (int)$catid, 'status' => 99), '*', 'id ASC');
		}
		// 挂钩点 内容读取之后
		...
		extract($data);
		pc_base::load_app_class('url')->show($id, '[page]', $catid, $rs['inputtime'], $prefix);
