...
class index {
	...
	public function init() {
		...
		if (is_mobile($siteid) && $sitelist[$siteid]['mobileauto'] || defined('IS_MOBILE') && IS_MOBILE) {
			if (!file_exists(PC_PATH.'templates'.DIRECTORY_SEPARATOR.$default_style.DIRECTORY_SEPARATOR.'mobile'.DIRECTORY_SEPARATOR.'index.html')) {
				define('ISMOBILE', 0);
	...
	public function show() {
		...
		if (is_mobile($siteid) && $sitelist[$siteid]['mobileauto'] || defined('IS_MOBILE') && IS_MOBILE) {
			if (!file_exists(PC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'mobile'.DIRECTORY_SEPARATOR.$template.'.html')) {
				...
				pc_base::load_app_func('global','mobile');
				define('ISMOBILE', 1);
				if($CAT['setting']['content_ishtml'] && $sitelist[$siteid]['mobilehtml']) {
					define('IS_HTML', 1);
				} else {
					define('IS_HTML', 0);
				}
	...
	public function lists() {
		...
		if($setting['disabled']) showmessage(L('category_not_exists'),'blank');
		//SEO
