...
class create_html extends admin {
	...
	public function update_urls() {
		...
		foreach ($table_list as $t) {
			$t = str_replace('_data_0', '_data_[tableid]', $t);
			$tables[$t] = $t;
		}
	...
	*/
	public function public_show_url() {
		...
		$psize = $pagesize ? $pagesize : 100; // 每页处理的数量
		...
		$url = '?m=content&c=create_html&a=public_show_url&modelid='.$modelid.'&pagesize='.$pagesize;
		// 获取栏目
		if ($catids) {
			$cat = array();
			foreach ($catids as $i) {
		...
		foreach ($data as $t) {
			...
			list($urls) = $this->url->show($t['id'], 0, $t['catid'], $t['inputtime'], dr_value(dr_cat_value($t['catid'], 'modelid'), $t['id'], 'prefix'), '', 'edit');
			//更新到数据库
			$this->db->update(array('url'=>$urls[0]),array('id'=>$t['id']));
		}
	...
	*/
	...
	public function public_tag_edit() {
		...
		$page = (int)$this->input->get('page');
		$pagesize = (int)$this->input->get('pagesize');
		$psize = $pagesize ? $pagesize : 10; // 每页处理的数量
		$total = (int)$this->input->get('total');
		$this->db->set_model($modelid);
		...
		$url = '?m=content&c=create_html&a=public_tag_edit&modelid='.$modelid.'&pagesize='.$pagesize;
	...
	public function public_thumb_edit() {
		...
		$url = '?m=content&c=create_html&a=public_thumb_edit&modelid='.$modelid.'&pagesize='.$pagesize;
	...
	public function public_desc_edit() {
		...
		$url = '?m=content&c=create_html&a=public_desc_edit&modelid='.$modelid.'&pagesize='.$pagesize;
	...
	public function public_cat_edit() {
		...
		$url = '?m=content&c=create_html&a=public_cat_edit&modelid='.$modelid.'&pagesize='.$pagesize;
		$url.= '&toid='.$toid;
		$where = array();
	...
	private function _get_field($bm) {
		...
		$fields = $this->db->get_fields($bm);
		if (!$fields) {
			dr_json(0, L('表['.$bm.']没有可用字段'));
		}

		$rt = array();
		foreach ($fields as $t => $r) {
			$rt[] = $t;
		}
	...
	public function public_replace_index() {
		...
		foreach ($tables as $table => $fields) {
			...
			} elseif (dr_in_array($fd, $this->db->get_primary($table))) {
				dr_json(0, implode('，', $this->db->get_primary($table)).L('主键不支持替换'));
			}
					...
					if ($mod) {
						...
						foreach ($mod as $t) {
							if ($t['issystem']) {
								...
								for ($i = 0;; $i ++) {
									if (!$this->db->table_exists($table.'_data_'.$i)) {
										break;
									}
	...
	public function public_field_index() {
		...
		$fields = $this->db->get_fields($this->db->db_tablepre.$table);
		if (!$fields) {
		...
		foreach ($fields as $t => $r) {
			if (!dr_in_array($t, $this->db->get_primary($this->db->db_tablepre.$table))) {
				$msg.= '<option value="'.$t.'">'.$t.($r['Comment'] ? '（'.$r['Comment'].'）' : '').'</option>';
			}
		}
		$msg.= '</select>';
