...
class content extends admin {
	...
	public function init() {
		...
		if(isset($param['catid']) && $param['catid'] && dr_cat_value($param['catid'], 'siteid')==$this->siteid) {
			...
			if (isset($param['keyword']) && $param['keyword'] != '') {
				...
				if ($param['field'] == 'id') {
				...
				} elseif ($this->db->field_exists($param['field']) && $param['keyword'] && substr_count($param['keyword'], ',') == 1 && preg_match('/[\+\-0-9\.]+,[\+\-0-9\.]+/', $param['keyword'])) {
					// BETWEEN 条件
					...
					if ($this->db->field_exists($param['field'])) {
	...
	public function public_check_title() {
		...
		$id = intval($this->input->get('id'));
		$modelid = dr_cat_value($catid, 'modelid');
		$cache = getcache('model_field_'.$modelid,'model');
		$this->db->set_model($modelid);
		...
		if ($is_ajax) {
			if($r) {
				exit(L('已经有相同的'.(isset($cache['title']['name']) ? $cache['title']['name'] : L('标题')).'存在'));
			}
		} else {
			if($r) {
				exit('1');
	...
	public function public_crop() {
		...
		if (IS_POST) {
			...
			$uploadedfile['isimage'] = dr_is_image($filetype) ? 1 : 0;
		...
		if ($steps) {
			...
			function ajax_manage(type) {
				if(type==1) {
					...
					$.post('?m=content&c=content&a=delete&ajax_preview=1&catid=".$catid."&steps=".$steps."&id=".$id."&pc_hash=".$pc_hash."',{dosubmit:1,".csrf_token().":'".csrf_hash()."'});
				}
