...
class sitemodel extends admin {
	private $input,$cache,$db,$menu_db,$cache_api,$field,$type_db,$sitemodel_field_db,$category_db;
	public $siteid;
	...
	public function edit() {
		if(IS_AJAX_POST) {
			...
			$this->field = $this->sitemodel_field_db->select(array('siteid'=>$this->siteid, 'modelid'=>$modelid, 'issystem'=>1, 'disabled'=>0),'*','','listorder ASC,fieldid ASC');
			...
			if (!$setting['search']['field']) {
				$setting['search']['field'] = 'title,keywords';
			}
			$info['setting'] = array2string($setting);
			$info['category_template'] = $setting['category_template'];
			$info['list_template'] = $setting['list_template'];
			...
			$search_field = $this->sitemodel_field_db->select(array('siteid'=>$this->siteid, 'modelid'=>$modelid, 'issystem'=>1, 'issearch'=>1, 'disabled'=>0),'*','','listorder ASC,fieldid ASC');
	...
	public function delete() {
		if(IS_AJAX_POST) {
			...
			for ($i = 0;; $i ++) {
				...
				if (!$this->db->table_exists($tablename_data)) {
					break;
				}
				$tablename_data = '';
	...
	function import(){
		if(IS_POST) {
			...
			if($this->db->table_exists($basic_table)) dr_admin_msg(0,L('operation_failure'),'?m=content&c=sitemodel&a=init');
