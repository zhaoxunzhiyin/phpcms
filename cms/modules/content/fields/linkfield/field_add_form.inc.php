...
$tbl_show = $db->list_tables($database[$pdo_name]['database']);
$web_tables = array();
$web_tables_info = array();
$pre = $database[$pdo_name]['tablepre'];
//排除系统关键信息数据表
$sys_tables = array($pre.'admin', $pre.'admin_login', $pre.'admin_panel', $pre.'admin_role', $pre.'admin_role_priv', $pre.'block', $pre.'block_history', $pre.'dbsource', $pre.'member_login', $pre.'log', $pre.'times', $pre.'urlrule', $pre.'session', $pre.'site', $pre.'queue', $pre.'release_point');
foreach($tbl_show as $k => $rs) {
	if(strpos($rs, $pre) === 0 && !in_array($rs, $sys_tables) && !preg_match('/'.$pre.'poster_(\d){6}/i',$rs)) {
		$web_tables[] = $rs;
	}
}
