	function images($field, $value, $fieldinfo) {
		...
		$string = load_js(JS_PATH.'h5upload/h5editor.js');
		$string .= load_css(JS_PATH.'jquery-ui/jquery-ui.min.css');
		$string .= load_js(JS_PATH.'jquery-ui/jquery-ui.min.js');
		$authkey = upload_key($this->input->get('siteid').",$upload_number,$upload_allowext,$upload_maxsize,$isselectimage,,,$watermark,$attachment,$image_reduce,$chunk");
		$p = dr_authcode(array(
		...
		$string .= "<input type=\"hidden\" name=\"info[{$field}]\" value=\"\"><label><button type=\"button\" onclick=\"javascript:h5upload('".SELF."', '{$field}_images', '".L('attachment_upload')."','{$field}','".($grid ? 'change_images' : 'change_files')."','{$p}','content','$this->catid','{$authkey}',".SYS_EDITOR.")\" class=\"btn green btn-sm\"> <i class=\"fa fa-plus\"></i> ".L('select_pic')."</button></label>";
		$string .= '<div class="bk10"></div>';
		$string .= '<fieldset class="blue pad-10">
        <legend>'.L('pic_list').'</legend>';
		$string .= ($ts ? $ts : '').'<div class="scroller_'.$field.'_files"><div class="'.($scroller ? 'scroller' : '').'" data-inited="0" data-initialized="1" data-always-visible="1" data-rail-visible="1">';
		if($grid) {
		...
		$string .= '</div></div></fieldset><script type="text/javascript">$("#fileupload_'.$field.'_files").sortable();dr_slimScroll_init(".scroller_'.$field.'_files", 300);</script>';
		return $string;
	}