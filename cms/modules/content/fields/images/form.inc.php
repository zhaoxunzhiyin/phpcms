	function images($field, $value, $fieldinfo) {
		extract($fieldinfo);
		$upload_allowext = 'jpg|gif|png|jpeg|svg|webp';
		$ts = $list_str = '';
		if($value) {
		...
		$string .= ($ts ? $ts : '').'<div class="scroller_'.$field.'_files"><div class="'.($scroller ? 'scroller' : '').'" data-inited="0" data-initialized="1" data-always-visible="1" data-rail-visible="1"><table class="table table-striped table-upload clearfix"><tbody id="'.$field.'" class="files scroller_body">'.$list_str.'</tbody></table></div></div>';
		$string .= '</fieldset><script type="text/javascript">$("#'.$field.'").sortable();dr_slimScroll_init(".scroller_'.$field.'_files", 300);</script>
		<div class="bk10"></div>';
		...
		$string .= $str."<label><button type=\"button\" onclick=\"javascript:h5upload('".SELF."', '{$field}_images', '".L('attachment_upload')."','{$field}','change_images','{$p}','content','$this->catid','{$authkey}',".SYS_EDITOR.")\" class=\"btn green btn-sm\"> <i class=\"fa fa-plus\"></i> ".L('select_pic')."</button></label>";
		return $string;
	}