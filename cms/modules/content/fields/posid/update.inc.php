			...
			if($this->input->get('a')=='add') {
				...
				foreach($value as $r) {
					if($r!='-1') {
						...
						$maxnum = $position_db->get_one(array('posid'=>$r));
						$delete_ids = $position_data_db->select(array('posid'=>$r),'id',(intval($maxnum['maxnum'])-1).",10000",'listorder DESC');
						if ($delete_ids) {
							foreach ($delete_ids as $k => $v) {
								$position_data_db->delete(array('id'=>$v['id'], 'posid'=>$r));
							}
						}
