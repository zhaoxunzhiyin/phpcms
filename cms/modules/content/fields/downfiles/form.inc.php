	function downfiles($field, $value, $fieldinfo) {
		...
		$string .= '<div class="scroller_'.$field.'_files"><div class="'.($scroller ? 'scroller' : '').'" data-inited="0" data-initialized="1" data-always-visible="1" data-rail-visible="1"><table class="table table-striped table-upload clearfix"><tbody id="'.$field.'" class="files scroller_body">'.$list_str.'</tbody></table></div></div>
		</fieldset><script type="text/javascript">$("#'.$field.'").sortable();dr_slimScroll_init(".scroller_'.$field.'_files", 300);</script>
		<div class="bk10"></div>';
		...
		$string .= $str."<label><button type=\"button\" onclick=\"javascript:h5upload('".SELF."', '{$field}_multifile', '".L('attachment_upload')."','{$field}','change_multifile','{$p}','content','$this->catid','{$authkey}',".SYS_EDITOR.")\" class=\"btn green btn-sm\"> <i class=\"fa fa-plus\"></i> ".L('multiple_file_list')."</button></label>";
		return $string;
	}
