...
<table cellpadding="2" cellspacing="1" width="98%">
    ...
    <tr<?php if(!$modelid || $modelid==-1 || $modelid==-2) {echo ' style="display: none;"';}?>> 
      <td>是否启用关联链接：</td>
      <td><div class="mt-radio-inline">
          <label class="mt-radio mt-radio-outline"><input type="radio" name="setting[enablekeylink]" value="1" <?php if($setting['enablekeylink']==1) echo 'checked';?>>是 <span></span></label>
          <label class="mt-radio mt-radio-outline"><input type="radio" name="setting[enablekeylink]" value="0" <?php if($setting['enablekeylink']==0) echo 'checked';?>> 否 <span></span></label>
          <input type="text" name="setting[replacenum]" value="<?php echo $setting['replacenum'];?>" size="4" class="input-text"> 替换次数 （留空则为替换全部）</div></td>
    </tr>
    <tr<?php if(!$modelid || $modelid==-1 || $modelid==-2) {echo ' style="display: none;"';}?>> 
      <td>关联链接方式：</td>
      <td><div class="mt-radio-inline">
          <label class="mt-radio mt-radio-outline"><input type="radio" name="setting[link_mode]" value="1" <?php if($setting['link_mode']==1) echo 'checked';?>> 关键字链接 <span></span></label>
