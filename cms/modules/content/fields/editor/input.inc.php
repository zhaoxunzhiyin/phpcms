	function editor($field, $value) {
		...
		if(($enablesaveimage || ($this->input->post('is_auto_down_img_'.$field) && $this->input->post('is_auto_down_img_'.$field))) && !defined('IS_COLL')) {
			$info_catid = $this->input->post('info');
			if (!$info_catid) {
				$info_catid = array();
			}
			$value = $this->download->download($value, $watermark, $attachment, $image_reduce, $info_catid['catid'] ? $info_catid['catid'] : (param::get_cookie('catid') ? param::get_cookie('catid') : 0));
		}
		if(intval($local_img) && !defined('IS_COLL')) {
			...
			$value = $this->download->upload_local($value, $local_watermark, $local_attachment, $local_image_reduce, $info_catid['catid'] ? $info_catid['catid'] : (param::get_cookie('catid') ? param::get_cookie('catid') : 0));
		}
		// 去除站外链接
