...
class url{
	...
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0,$mobile = 0) {
		...
		if($upgrade || ($this->input->post('upgrade') && defined('IS_ADMIN') && IS_ADMIN && $this->input->post('upgrade'))) {
			...
			$urls = str_replace(array('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$prefix}','{$id}','{$page}'),array($categorydir,$catdir,$year,$month,$day,$catid,$prefix,$id,$page),$urlrule);
			$showurls = str_replace(array('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$prefix}','{$id}','{$page}'),array($categorydir,$catdir,$year,$month,$day,$catid,$prefix,$id,'{page}'),$urlrule);
