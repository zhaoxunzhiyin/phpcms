...
class url {
	...
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0,$mobile = 0) {
		...
		!$content_ishtml && $my_url && strpos($my_url, '.php') === false && dr_save_rewrite_routes(ltrim(WEB_PATH, '/').$my_url, trim($php_url, '/'));
	...
	public function category_url($catid, $page = 1) {
		...
		$page > 1 && !$setting['ishtml'] && strpos($url, '.php') === false && dr_save_rewrite_routes(ltrim(WEB_PATH, '/').$url, trim($php_url, '/'));
		!$setting['ishtml'] && strpos($my_url, '.php') === false && dr_save_rewrite_routes(ltrim(WEB_PATH, '/').$my_url, trim($php_url, '/'));
		return $url;
	}
	/**
	 * 生成列表页分页地址
