...
class RSSBuilder extends RSSBase {
	...
	*/
	function setEncoding($encoding = '') {
		if (!isset($this->encoding)) {
			$this->encoding = (string) ((dr_strlen(trim($encoding)) > 0) ? trim($encoding) : 'UTF-8');
		} // end if
	} // end function
	...
	*/
	function setAbout($about = '') {
		if (!isset($this->about) && dr_strlen(trim($about)) > 0) {
	...
	*/
	function setTitle($title = '') {
		if (!isset($this->title) && dr_strlen(trim($title)) > 0) {
	...
	*/
	function setDescription($description = '') {
		if (!isset($this->description) && dr_strlen(trim($description)) > 0) {
	...
	*/
	function setPublisher($publisher = '') {
		if (!isset($this->publisher) && dr_strlen(trim($publisher)) > 0) {
	...
	*/
	function setCreator($creator = '') {
		if (!isset($this->creator) && dr_strlen(trim($creator)) > 0) {
	...
	*/
	function setDate($date = '') {
		if (!isset($this->date) && dr_strlen(trim($date)) > 0) {
	...
	*/
	function setRights($rights = '') {
		if (!isset($this->rights) && dr_strlen(trim($rights)) > 0) {
	...
	*/
	function setCoverage($coverage = '') {
		if (!isset($this->coverage) && dr_strlen(trim($coverage)) > 0) {
	...
	*/
	function setContributor($contributor = '') {
		if (!isset($this->contributor) && dr_strlen(trim($contributor)) > 0) {
	...
	*/
	function setImageLink($image_link = '') {
		if (!isset($this->image_link) && dr_strlen(trim($image_link)) > 0) {
	...
	*/
	function setPeriod($period = '') {
		if (!isset($this->period) && dr_strlen(trim($period)) > 0) {
			switch ($period) {
				case 'hourly':
				case 'daily':
				case 'weekly':
	...
	*/
	function setFrequency($frequency = '') {
		if (!isset($this->frequency) && dr_strlen(trim($frequency)) > 0) {
	...
	*/
	function setBase($base = '') {
		if (!isset($this->base) && dr_strlen(trim($base)) > 0) {
	...
	*/
	function createOutputV091() {
		...
		if (dr_strlen($this->title) > 0) {
			$this->output .= (string) '<title>' . $this->title . '</title>' . "\n";
		} // end if

		if (dr_strlen($this->image_link) > 0) {
		...
		if (count($this->getItemList()) > 0) {
			foreach ($this->getItemList() as $id) {
				...
				if (dr_strlen($item->getTitle()) > 0 && dr_strlen($item->getLink()) > 0) {
					...
					if ($this->use_dc_data === TRUE && dr_strlen($item->getSubject()) > 0) {
						$this->output .= (string) '<dc:subject>' . $item->getSubject() . '</dc:subject>' . "\n";
					} // end if

					if ($this->use_dc_data === TRUE && dr_strlen($item->getDate()) > 0) {
						$this->output .= (string) '<dc:date>' . $item->getDate() . '</dc:date>' . "\n";
					} // end if

					if (dr_strlen($item->getImage()) > 0) {
	...
	*/
	function createOutputV200() {
		...
		if (dr_strlen($this->category) > 0) {
			$this->output .= (string) '<category>' . $this->category . '</category>' . "\n";
		} // end if

		if (dr_strlen($this->cache) > 0) {
		...
		if (count($this->getItemList()) > 0) {
			foreach ($this->getItemList() as $id) {
				...
				if (dr_strlen($item->getTitle()) > 0 && dr_strlen($item->getLink()) > 0) {
					...
					if (dr_strlen($item->getAuthor()) > 0) {
						$this->output .= (string) '<author>' . $item->getAuthor() . '</author>' . "\n";
					} // end if

					if (dr_strlen($item->getComments()) > 0) {
	...
	*/
	function outputRSS($version = '') {
		...
		$this->output = '<?xml version="1.0" encoding="' . $this->encoding . '"?>' . "\n" .
						'<!--  RSS generated by cms.cn RSS Builder [' .  date('Y-m-d H:i:s')  .']  --> '. "\n"  . $this->output;
		echo $this->output;
	} // end function
	...
	*/
	function getRSSOutput($version = '') {
		...
		return (string) '<?xml version="1.0" encoding="' . $this->encoding . '"?>' . "\n" .
						'<!--  RSS generated by cms.cn RSS Builder [' .  date('Y-m-d H:i:s')  .']  --> ' . $this->output;
	} // end function
	/**#@-*/
...
class RSSItem extends RSSBase {
	...
	*/
	function setLink($link = '') {
		if (!isset($this->link) && dr_strlen(trim($link)) > 0) {
	...
	*/
	function setSubject($subject = '') {
		if (!isset($this->subject) && dr_strlen(trim($subject)) > 0) {
	...
	*/
	function setAuthor($author = '') {
		if (!isset($this->author) && dr_strlen(trim($author)) > 0) {
	...
	*/
	function setComments($comments = '') {
		if (!isset($this->comments) && dr_strlen(trim($comments)) > 0) {
	...
	*/
	function setImage($image = '') {
		if (!isset($this->image) && dr_strlen(trim((string)$image)) > 0) {
