...
class html {
	...
	public function category($catid, $page = 0, $maxsize = 0) {
		...
		if($type==0) {
			...
			//绑定域名时，设置为空
			if($isdomain) $data['categorydir'] = $data['parentdir'] = $data['catdir'] = '';
			
			$data['page'] = $page;
			$categoryurl = explode('~', get_url_value($data, $urlrules[0], '', 0).'~'.get_url_value($data, $urlrules[1], '', 0));
			
			pc_base::load_sys_class('service')->assign(array(
				'fix_html_now_url' => $page == 1 ? ($isdomain ? substr((string)$CAT['url'], 0, -1).$categoryurl[0] : siteurl($this->siteid).$categoryurl[0]) : ($isdomain ? substr((string)$CAT['url'], 0, -1).$categoryurl[1] : siteurl($this->siteid).$categoryurl[1]),
				'urlrule' => $urlrule,
			));
		} else {
			//单网页
