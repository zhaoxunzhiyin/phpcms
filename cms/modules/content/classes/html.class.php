...
class html {
	...
	public function show($file, $data = '', $array_merge = 1,$action = 'add',$upgrade = 0) {
		...
		if ($this->siteid==1) {
			...
			$queue_file = $this->html_root.'/'.dr_site_info('dirname', $this->siteid).$file;
		}
	...
	public function category($catid, $page = 0, $maxsize = 0) {
		...
		if($this->siteid!=1) {
			...
			if($create_to_html_root) {
				$mobile_base_file = $site_dir.$this->mobile_root.$base_file;
				$base_file = $site_dir.$base_file;
			} else {
				$mobile_base_file = $site_dir.$this->mobile_root.$this->html_root.$base_file;
		...
		if($create_to_html_root) {
			...
			$mobile_file = substr($this->html_root,1).$mobile_base_file;
			//添加到发布点队列
		...
		if($type==0) {
			...
			$day = date('d',$time);
			//URL规则
			$urlrule = implode('~', str_replace(array('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$page}'),array($this->get_categorydir($catid),$catdir,$year,$month,$day,$catid,'{page}'),$urlrules));
