...
class html {
	...
	public function show($file, $data = '', $array_merge = 1,$action = 'add',$upgrade = 0) {
		...
		if (!$output_data) {
			...
			foreach ($this->form_cache['field'] as $t) {
				if ($t['formtype'] == 'redirect' && $data[$t['field']]) {
					...
					ob_start();
					pc_base::load_sys_class('service')->assign('goto_url', $data[$t['field']]);
		...
		$output_data['style'] = $data['style'];
		$output_data['tableid'] = $data['tableid'];
		$output_data['sysadd'] = $data['sysadd'];
		$output_data['paginationtype'] = $data['paginationtype'];
		$output_data['maxcharperpage'] = $data['maxcharperpage'];
		$output_data['paytype'] = $data['paytype'];
		$output_data['cat'] = $CAT;
		...
		$output_data['_inputtime'] = $data['inputtime'];
		$output_data['_updatetime'] = $data['updatetime'];
		$this->db->set_model($modelid);
		//上一页
		$output_data['previous_page'] = $this->db->get_one(array('catid' => (int)$catid, 'id<' => (int)$id, 'status' => 99), '*', 'id DESC');
		if(isset($this->form_cache['setting']['previous']) && $this->form_cache['setting']['previous'] && !$output_data['previous_page']) {
			$output_data['previous_page'] = $this->db->get_one(array('catid' => (int)$catid, 'status' => 99), '*', 'id DESC');
		}
		//下一页
		$output_data['next_page'] = $this->db->get_one(array('catid' => (int)$catid, 'id>' => (int)$id, 'status' => 99), '*', 'id ASC');
		if(isset($this->form_cache['setting']['previous']) && $this->form_cache['setting']['previous'] && !$output_data['next_page']) {
			$output_data['next_page'] = $this->db->get_one(array('catid' => (int)$catid, 'status' => 99), '*', 'id ASC');
		}
		// 挂钩点 内容读取之后
		...
		if($paginationtype!=0) {
			...
			if($CONTENT_POS !== false) {
				...
				for($i=1; $i<=$pagenumber; $i++) {
					...
					list($pageurls[$i], $showurls[$i]) = $this->url->show($id, $i, $catid, $inputtime, $data['prefix'], '', 'edit', $upgrade);
				}
				...
				foreach ($pageurls as $page=>$urls) {
					...
					pc_base::load_sys_class('service')->assign(array(
						...
						'titles' => $titles,
						'pages' => $pages,
						'fix_html_now_url' => preg_match('/^(http|https):\/\//', (string)$urls[1]) ? $url : siteurl($this->siteid).$urls[1],
					));
					ob_start();
		...
		pc_base::load_sys_class('service')->assign(array(
			...
			'titles' => $titles,
			'pages' => $pages,
			'fix_html_now_url' => preg_match('/^(http|https):\/\//', (string)$url) ? $url : siteurl($this->siteid).$url,
		));
		ob_start();
		pc_base::load_sys_class('service')->init('pc');
		pc_base::load_sys_class('service')->display('content', $template);
		...
		if(dr_site_info('mobilehtml', $this->siteid)==1) {
			...
			pc_base::load_sys_class('service')->assign(array(
				...
				'titles' => $titles,
				'pages' => $pages,
				'fix_html_now_url' => preg_match('/^(http|https):\/\//', (string)$url) ? $url : siteurl($this->siteid, 1).$url,
			));
			ob_start();
			pc_base::load_sys_class('service')->init('mobile');
			pc_base::load_sys_class('service')->display('content', $template);
	...
	public function category($catid, $page = 0, $maxsize = 0) {
		...
		$base_file = $this->url->get_list_url($setting['category_ruleid'], get_categorydir($catid), get_parentdir($catid), $catdir, $catid, $page);
		$base_file = '/'.$base_file;
		...
		if($type==0) {
			...
			$data['page'] = '{page}';
			//URL规则
			$urlrule = get_url_value($data, $urlrules[0], '', 0).'~'.get_url_value($data, $urlrules[1], '', 0);
			
			//绑定域名时，设置为空
			if($category['isdomain']) $data['categorydir'] = $data['parentdir'] = $data['catdir'] = '';
			
			$data['page'] = $page;
			$categoryurl = explode('~', get_url_value($data, $urlrules[0], '', 0).'~'.get_url_value($data, $urlrules[1], '', 0));
			
			pc_base::load_sys_class('service')->assign(array(
				'fix_html_now_url' => $page == 1 ? ($category['isdomain'] ? substr((string)$CAT['url'], 0, -1).$categoryurl[0] : siteurl($this->siteid).$categoryurl[0]) : ($category['isdomain'] ? substr((string)$CAT['url'], 0, -1).$categoryurl[1] : siteurl($this->siteid).$categoryurl[1]),
				'urlrule' => $urlrule,
			));
		} else {
			//单网页
		...
		if ($type != 2 && $child && $setting['getchild']) {
			...
			if ($temp) {
				foreach ($temp as $i) {
					...
					if ($i != $catid && $row['type'] != 2 && !$row['setting']['getchild']) {
						$catid = $i;
						ob_start();
						pc_base::load_sys_class('service')->assign('goto_url', $row['url']);
						pc_base::load_sys_class('service')->admin_display('go', 'admin');
			...
			if(dr_site_info('mobilehtml', $this->siteid)==1) {
				if($type==0) {
					...
					pc_base::load_sys_class('service')->assign(array(
						'fix_html_now_url' => $page == 1 ? ($category['isdomain'] ? substr((string)$CAT['url'], 0, -1).$url_arr[0] : siteurl($this->siteid, 1).(dr_site_info('mobilemode', $this->siteid)==0 ? str_replace('mobile/', '', $url_arr[0]) : $url_arr[0])) : ($category['isdomain'] ? substr((string)$CAT['url'], 0, -1).$url_arr[1] : siteurl($this->siteid, 1).(dr_site_info('mobilemode', $this->siteid)==0 ? str_replace('mobile/', '', $url_arr[1]) : $url_arr[1])),
						'urlrule' => $urlrule,
					));
				} else {
					pc_base::load_sys_class('service')->assign(array(
