...
class type {
	...
	public function init() {
		...
		if (is_mobile($siteid) && dr_site_info('mobileauto', $siteid) || defined('IS_MOBILE') && IS_MOBILE) {
			if (!file_exists(TPLPATH.$default_style.DIRECTORY_SEPARATOR.'mobile'.DIRECTORY_SEPARATOR.'type.html')) {
				define('ISMOBILE', 0);
				define('IS_HTML', 0);
				...
				define('ISMOBILE', 1);
				define('IS_HTML', 0);
				pc_base::load_sys_class('service')->display('mobile','type',$default_style);
			}
		}else{
			define('ISMOBILE', 0);
			define('IS_HTML', 0);
