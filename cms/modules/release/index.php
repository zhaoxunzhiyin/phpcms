...
class index extends admin {
	private $input, $db, $queue, $siteid, $site, $point;
	public function __construct() {
		...
		$this->queue->delete(array('times<='=>SYS_TIME-2592000));
	}
	...
	public function public_sync() {
		...
		if (!empty($ids)) {
			...
			$data = $this->queue->select($sql, 'id, type, path');
		}else {
			if (empty($total)) {
				$total = $this->queue->count(array("siteid"=>$this->siteid, "status".($id+1)=>$statuses));
			}
			$totalpage = ceil($total/$pagesize);
			$data = $this->queue->select(array("siteid"=>$this->siteid, "status".($id+1)=>$statuses), 'id, type, path', $pagesize);
		}
		...
		if(is_array($data) && !empty($data)) if ($ftps->connect($release_point['host'], $release_point['username'], $release_point['password'], $release_point['port'], $release_point['pasv'], $release_point['ssl'])) {
			...
			foreach ($data as $v) {
				...
				$this->queue->update(array('status'.($id+1)=>$status, 'times'=>SYS_TIME), array('id'=>$v['id']));
			}
		} else {
	...
	public function failed() {
		...
		$list = $this->queue->listinfo($sql, 'id desc', $page, SYS_ADMIN_PAGESIZE);
