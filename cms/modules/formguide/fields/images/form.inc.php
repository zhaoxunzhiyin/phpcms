	function images($field, $value, $fieldinfo) {
		...
		$string .= $str."<input type=\"hidden\" name=\"info[{$field}]\" value=\"\"><label><button type=\"button\" onclick=\"javascript:h5upload('".SELF."', '{$field}_images', '".L('attachment_upload')."','{$field}','".($grid ? 'change_images' : 'change_files')."','{$p}','content','$this->catid','{$authkey}',".SYS_EDITOR.")\" class=\"btn green btn-sm\"> <i class=\"fa fa-plus\"></i> ".L('select_pic')."</button></label>";
		return $string;
	}