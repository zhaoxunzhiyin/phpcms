	function textarea($field, $value, $fieldinfo) {
		...
		return "<textarea name='info[{$field}]' id='$field' class='form-control".(isset($css) && $css ? ' '.$css : '')."' style='width:{$width}".(is_numeric($width) ? 'px' : '').";height:{$height}px;' $formattribute $this->no_allowed>".code2html($value)."</textarea>";
	}
