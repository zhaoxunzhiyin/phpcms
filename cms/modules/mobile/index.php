...
class index {
	private $input,$cache,$db,$_userid,$_username,$_groupid,$siteid,$module,$category,$categorys,$category_priv_db,$category_setting,$sitemodel,$form_cache,$page_db,$url,$category_db,$hits_db;
			...
			if ($temp) {
				foreach ($temp as $i) {
					...
					if ($i != $catid && $row['type'] != 2 && !$row['setting']['getchild']) {
						...
						$category = $row;
						$url = $category['url'];
						$url = str_replace(array(dr_site_info('domain', $siteid), 'm=content'), array(dr_site_info('mobile_domain', $siteid), 'm=mobile'), $url);
						param::redirect($url);
						exit;
						break;
					}
			...
			if (dr_site_info('mobilehtml', $siteid)==1) {
				...
				if (!dr_site_info('mobilemode', $siteid)) {
					if ($setting['ishtml'] && !$CAT['sethtml']) {
						$urlrules = str_replace('|', '~'.SYS_MOBILE_ROOT.SYS_HTML_ROOT.'/', SYS_MOBILE_ROOT.SYS_HTML_ROOT.'/'.$urlrules[$setting['category_ruleid']]);
					} else {
						$urlrules = str_replace('|', '~'.SYS_MOBILE_ROOT.'/', SYS_MOBILE_ROOT.'/'.$urlrules[$setting['category_ruleid']]);
					}
				} else {
					if ($setting['ishtml'] && !$CAT['sethtml']) {
	...
	public function show() {
		...
		define('ISMOBILE', 1);
		if($this->category_setting['content_ishtml'] && dr_site_info('mobilehtml', $siteid)) {
			define('IS_HTML', 1);
		} else {
			define('IS_HTML', 0);
		}
	...
	public function maps() {
		...
		pc_base::load_sys_class('service')->display('mobile', 'maps', $default_style);
	}
	...
	public function json_list() {
		...
		//根据关键字搜索
			...
			if(isset($MODEL[$modelid])) {
				...
				$this->db->set_model($modelid);
				$result = $this->db->select(array('keywords'=>'%'.$keywords.'%', 'id<>'=>$id),'id,catid,title,url',10);
				if(!empty($result)) {
					$data = array();
					foreach($result as $rs) {
						$rs['url'] = show_url($rs['url'], $rs['catid'], $rs['id']);
						$data[] = $rs;
					}
