...
class index {
	private $input,$cache,$db,$_userid,$_username,$_groupid,$siteid,$category,$category_priv_db,$category_setting,$sitemodel,$form_cache,$page_db,$url,$category_db;
	...
	private function _getCategoryId($catdir){
		...
		$result = $this->category_db->select(array('catdir'=>$dirname, 'siteid'=>$this->siteid));
		foreach($result as $r){
			if ($r['parentid']) {
