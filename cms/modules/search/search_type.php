...
class search_type extends admin {
	private $input,$db,$siteid,$model,$yp_model,$cache_api;
	...
	public function add() {
		if(IS_POST) {
			...
			$info['module'] = 'search';
			!$module && dr_admin_msg(0,L('请选择').L('select_module_name'), array('field' => 'module'));
			if($module=='content') {
				!$info['modelid'] && dr_admin_msg(0,L('请选择').L('select_model_name'), array('field' => 'modelid'));
				$info['modelid'] = intval($info['modelid']);
				$info['typedir'] = $module;
			} elseif($module=='yp') {
				!$info['yp_modelid'] && dr_admin_msg(0,L('请选择').L('select_model_name'), array('field' => 'yp_modelid'));
