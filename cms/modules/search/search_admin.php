...
class search_admin extends admin {
	private $input,$siteid,$db,$menu_db,$module_db,$type_db,$cache_api;
		...
		if($this->input->get('dosubmit')) {
			...
			foreach ($types as $_k=>$_v) {
				if($key==$_k) {
					...
					$num = intval($this->input->get('num')) ? intval($this->input->get('num')) : 10;
					...
					foreach ($datas as $id=>$r) {
						$this->db->update_search($typeid ,$id, $r['fulltextcontent'],$r['title'],$r['adddate'], 1, $num);
					}
					$page++;
					if($pages>=$page) dr_admin_msg(1,"正在更新 <span style='color:#ff0000;'>{$_v['name']}</span> - 总数：{$total} - 当前第 <font color='red'>{$page}</font> 页","?m=search&c=search_admin&a=createindex&menuid={$this->input->get('menuid')}&page={$page}&total={$total}&key={$key}&num={$num}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
					$key++;
					dr_admin_msg(1,"开始更新：<span style='color:#ff0000;'>{$_v['name']}</span> - 总数：{$total}条","?m=search&c=search_admin&a=createindex&menuid={$this->input->get('menuid')}&page=1&key={$key}&num={$num}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
	...
	public function public_test_sphinx() {
		$sphinxhost = !empty($this->input->post('sphinxhost')) ? $this->input->post('sphinxhost') : dr_json(0, L('hostempty'));
		$sphinxport = !empty($this->input->post('sphinxport')) ? intval($this->input->post('sphinxport')) : dr_json(0, L('portempty'));
		$fp = @fsockopen($sphinxhost, $sphinxport, $errno, $errstr , 2);
		if (!$fp) {
			dr_json(0, $errno.':'.$errstr);
		} else {
			dr_json(1, L('testsuccess'));
		}
	}
