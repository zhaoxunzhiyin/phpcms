...
class index {
	...
	public function init() {
		...
		dr_search_url($search['params'], 'keyword', '');
		...
		if($get['keyword']) {
			...
			&& $get['page'] <= 1) {
				...
				if (!$ctime) {
					...
					dr_msg(0, L('search_minrefreshtime',array('min'=>$setting['search_time']),'content'), dr_search_url($search['params'], 'keyword', ''));
				} else {
					$this->cache->set_auth_data($cname, SYS_TIME);
				}
			}
			...
			if($typeid) {
				...
				$datas = $this->db->listinfo("`siteid`= '$siteid' $sql_time AND `data` like '%".$this->db->escape($keyword)."%'", 'searchid DESC', $page, $pagesize);
			...
			$datas && $totalnums ? $pages = pages($totalnums, $page, $pagesize, dr_search_url($search['params'], 'page', '{$page}')) : '';
