...
class index {
	private $input, $setting, $catid, $contentid, $siteid, $mood_id;
	public function __construct() {
		$this->input = pc_base::load_sys_class('input');
		$this->setting = getcache('mood_program', 'commons');
		
		
		$this->mood_id = $this->input->get('id') ? $this->input->get('id') : '';
	...
	public function init() {
		...
		pc_base::load_sys_class('service')->display('mood', 'index');
		$html = ob_get_contents();
		ob_clean();
		echo format_js($html);exit();
	}
	
	//提交选中
	public function post() {
		$callback = $this->input->get('callback');
		if (isset($callback) && !preg_match('/^[a-zA-Z_][a-zA-Z0-9_]+$/', $callback)) unset($callback);
		$mood_id =& $this->mood_id;
		$setting =& $this->setting;
		...
		if (in_array($this->mood_id, $cookie)) {
			...
			$mood_db = pc_base::load_model('mood_model');
			$key = $this->input->get('k') && intval($this->input->get('k')) ? intval($this->input->get('k')) : '';
	...
	protected function _show_result($status = 0, $msg = '') {
		...
		exit(trim_script($this->input->get('callback')).'('.json_encode(array('status'=>$status, 'data'=>$msg)).')');
	}
}