		...
		$('#file_upload').fileupload({
			...
			add: function (e, data) {
				...
				if(numItems + myItems".($ct ? ' + '.$ct : '')." > ".$file_upload_limit."){
					dr_tips(0, '".L('att_upload_num', array('file_num'=>$file_upload_limit)).(CI_DEBUG ? '（可在自定义字段中设置本字段的个数值）' : '')."');
					return false;
				}
				data.submit();
			},
				...
				if(json.ext == 1) {
					var img = '<span class=\"checkbox\"></span><input type=\"checkbox\" class=\"checkboxes\" name=\"ids[]\" value=\"'+json.id+'\" /><a href=\"javascript:preview(\''+json.url+'\');\" class=\"on\"><img src=\"'+json.url+'\" id=\"'+json.id+'\" path=\"'+json.url+'\" size=\"'+json.size+'\" filename=\"'+json.name+'\"/></a><i class=\"size\">'+json.size+'</i><i class=\"name\">'+json.name+'</i><div class=\"upload-del\" onclick=\"dr_file_delete(\'?m=attachment&c=manage&a=delete\', this, '+json.id+', 1);return false;\"><i class=\"fa fa-trash\"></i></div>';
				} else {
					var img = '<span class=\"checkbox\"></span><input type=\"checkbox\" class=\"checkboxes\" name=\"ids[]\" value=\"'+json.id+'\" /><a href=\"javascript:preview(\''+json.url+'\');\" class=\"on\"><img src=\"".WEB_PATH."api.php?op=icon&fileext='+json.ext+'\" id=\"'+json.id+'\" size=\"'+json.size+'\" path=\"'+json.url+'\" filename=\"'+json.name+'\"/></a><i class=\"size\">'+json.size+'</i><i class=\"name\">'+json.name+'</i><div class=\"upload-del\" onclick=\"dr_file_delete(\'?m=attachment&c=manage&a=delete\', this, '+json.id+', 1);return false;\"><i class=\"fa fa-trash\"></i></div>';
				}
				$.get('".SELF."?m=attachment&c=attachments&a=h5upload_json&aid='+json.id+'&src='+json.url+'&filename='+json.name+'&size='+json.size);
				$('#fileupload_files').append('<div class=\"col-md-2 col-sm-2 col-xs-6 upload-item\"><div id=\"attachment_'+json.id+'\" class=\"files_row on\" onclick=\"javascript:att_cancel(this)\"></div></div>');
