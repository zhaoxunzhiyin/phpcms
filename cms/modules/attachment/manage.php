...
class manage extends admin {
	...
	public function init() {
		...
		if(!empty($datas)) {
			foreach($datas as $r) {
				$remotes = get_cache('attachment', $r['remote']);
				if ($remotes) {
					if (is_dir(SYS_UPLOAD_PATH.$remotes['value']['path'])) {
						// 相对路径
						$file = SYS_UPLOAD_PATH.$remotes['value']['path'];
					} else if (is_dir(CMS_PATH.$remotes['value']['path'])) {
						// 相对路径
						$file = CMS_PATH.$remotes['value']['path'];
					} else {
						$file = $remotes['value']['path'];
					}
				} else {
					$file = SYS_UPLOAD_PATH;
				}
				$thumb = glob(dirname($file.$r['filepath']).'/thumb_*'.basename($r['filepath']));
				$rs['aid'] = $r['aid'];
				if(!$r['remote']) {
		...
		if($info) {
			...
			if ($remote) {
				if (is_dir(SYS_UPLOAD_PATH.$remote['value']['path'])) {
					// 相对路径
					$file = SYS_UPLOAD_PATH.$remote['value']['path'];
				} else if (is_dir(CMS_PATH.$remote['value']['path'])) {
					// 相对路径
					$file = CMS_PATH.$remote['value']['path'];
				} else {
					$file = $remote['value']['path'];
				}
				$url = $remote['url'];
			} else {
				$file = SYS_UPLOAD_PATH;
			...
			$infos = glob(dirname($file.$info['filepath']).'/thumb_*'.basename($info['filepath']));
			foreach ($infos as $n=>$thumb) {
				$thumbs[$n]['thumb_url'] = str_replace($file, $url, $thumb);
