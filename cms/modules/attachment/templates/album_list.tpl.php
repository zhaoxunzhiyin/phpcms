...
<div class="row">
    ...
    <input type="hidden" value="<?php echo $this->input->get('is_iframe');?>" name="is_iframe">
    ...
    <?php if (is_pc()) {?><div class="col-md-3 text-right margin-bottom-20">
...
<div class="col-md-2 col-sm-2 col-xs-6 upload-item">
    <div class="files_row tooltips" data-original-title="<?php echo $r['filename']?>&nbsp;&nbsp;<?php echo format_file_size($r['filesize'])?>"<?php if (is_mobile()) {?> id="attachment_<?php echo $r['aid']?>" onclick="javascript:att_cancel(this)"<?php }?>>
        <span class="checkbox"></span>
        <input type="checkbox" class="checkboxes" name="ids[]" value="<?php echo $r['aid']?>" />
        <a href="javascript:preview('<?php echo dr_get_file_url($r)?>');"><img class="rs-load" src="<?php echo JS_PATH;?>layer/theme/default/loading-1.gif" rs-src="<?php echo $r['src']?>" id="<?php echo $r['aid']?>" path="<?php echo dr_get_file_url($r)?>" size="<?php echo format_file_size($r['filesize'])?>" filename="<?php echo $r['filename']?>"/></a>
        <i class="size"> <?php echo format_file_size($r['filesize'])?> </i>
        <i class="name"><?php echo $r['filename']?></i>
        <div class="upload-del" onclick="dr_file_delete('?m=attachment&c=manage&a=delete', this, <?php echo $r['aid'];?>);return false;"><i class="fa fa-trash"></i></div>
    </div>
</div>
<?php } ?>
</div>
