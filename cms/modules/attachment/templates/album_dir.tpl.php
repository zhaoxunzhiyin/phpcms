...
<?php if ($dir !='' && $dir != '.'):?>
<tr>
<td align="left"><a href="<?php echo '?m=attachment&c=attachments&a=album_dir&args='.$this->input->get('args').'&authkey='.$this->input->get('authkey').'&dir='.stripslashes(dirname($dir)).'&is_iframe=1'?>"><img src="<?php echo IMG_PATH?>folder-closed.png" /><?php echo L("parent_directory")?></td></a>
</tr>
<?php endif;?>
<?php 
if(is_array($list)):
...
<tr>
<?php if (is_dir($v)) {
	echo '<td align="left"><img src="'.IMG_PATH.'folder-closed.png" /> <a href="?m=attachment&c=attachments&a=album_dir&args='.$this->input->get('args').'&authkey='.$this->input->get('authkey').'&dir='.($this->input->get('dir') && !empty($this->input->get('dir')) ? stripslashes($this->input->get('dir')).'/' : '').$filename.'&is_iframe=1"><b>'.$filename.'</b></a></td>';
} else {
