...
class attachments {
	private $input,$cache,$att_db,$upload,$userid,$rid,$grouplist,$roleid,$isadmin,$groupid,$sitedb,$cookie_att;
	...
	public function check_upload_auth($editor = 0) {
		...
		$error = '';
		if (!IS_API && defined('SYS_CSRF') && SYS_CSRF && csrf_hash() != (string)$this->input->get('token')) {
			$error = L('跨站验证禁止上传文件');
	...
	public function album_load() {
		...
		$where = "fileext in (".$s_str.") AND module<>'member' AND siteid=".$siteid;
		...
		foreach($infos as $n=>$v){
			...
			if(dr_is_image($ext)) {
				...
				$infos[$n]['src']=WEB_PATH.'api.php?op=icon&fileext='.$ext;
			}
		}
			...
			if(is_array($att) && !empty($att)) {
				foreach ($att as $n=>$v) {
					...
					if(dr_is_image($ext)) {
						...
						$att[$n]['fileimg']=WEB_PATH.'api.php?op=icon&fileext='.$ext;
						$att[$n]['filename']=urldecode($v['filename']);
					}
