...
<?php } else {?>
    <div class="portlet-title tabbable-line">
        <ul class="nav nav-tabs" style="float:left;">
            <li class="active">
                <a data-toggle="tab_0"<?php if (is_mobile()) {echo ' onmouseover="layer.tips(\''.L('database_export').'\',this,{tips: [1, \'#fff\']});" onmouseout="layer.closeAll();"';}?>> <i class="fa fa-th-large"></i> <?php if (!is_mobile()) {echo L('database_export');}?> </a>
            </li>
        </ul>
    </div>
...
$(function() {
    ...
    $(document).on("click", ".btn-backup", function () {
        ...
        <?php if (!is_mobile()) {?>$(this).html('正在发送备份请求...');<?php }?>
        $.ajax({
            type: "POST",
            dataType: "json",
            ...
            success: function(json) {
                ...
                if (json.code == 1) {
                    ...
                    <?php if (!is_mobile()) {?>$('.btn-backup').html(json.msg + '开始备份，请不要关闭本页面！');<?php }?>
                    backup(json.data.tab);
                } else {
                    $('.btn-backup').attr('disabled', false);
                    <?php if (!is_mobile()) {?>$('.btn-backup').html('<?php echo L('backup_starting');?>');<?php }?>
                    dr_tips(json.code, json.msg);
                }
                return false;
            },
    ...
    function backup(tab, code) {
        ...
        $.get('?m=admin&c=database&a=backup&menuid=<?php echo $this->input->get('menuid');?>&pc_hash='+pc_hash, tab, function (json) {
            if (json.code) {
                ...
                if (!json.data.tab) {
                    $('.btn-backup').attr('disabled', false);
                    <?php if (!is_mobile()) {?>$('.btn-backup').html("备份完成，点击重新备份");<?php }?>
                    if (json.data.url) {
                ...
                dr_tips(json.code, json.msg);
                $('.btn-backup').attr('disabled', false);
                <?php if (!is_mobile()) {?>$('.btn-backup').html("立即备份");<?php }?>
            }
        }, "json");
    }
