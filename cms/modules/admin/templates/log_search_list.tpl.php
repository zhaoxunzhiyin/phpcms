...
<table width="100%" cellspacing="0" class="search-form">
    <tbody>
		<tr>
		<td><div class="explain-col"><?php echo L('module')?>: <?php echo form::select($module_arr,'','name="search[module]"',$default)?> 用户名:  <input type="text" value="<?php echo $this->input->get('search')['username'];?>" class="input-text" name="search[username]" size='10'>  时 间:  <?php echo form::date('search[start_time]',$this->input->get('search')['start_time'],'1')?> 至   <?php echo form::date('search[end_time]',$this->input->get('search')['end_time'],'1')?>    <input type="submit" value="确定搜索" class="button" name="dosubmit">
		</div>
		</td>
		</tr>
    </tbody>
        ...
        <thead>
            ...
            <th width="35" align="center" class="myselect">
                    <label class="mt-table mt-checkbox mt-checkbox-single mt-checkbox-outline">
                        <input type="checkbox" class="group-checkable" value="" id="check_box" onclick="selectall('logid[]');" />
                        <span></span>
                    </label></th>
...
?>
    ...
    <td align="center" class="myselect">
                    <label class="mt-table mt-checkbox mt-checkbox-single mt-checkbox-outline">
                        <input type="checkbox" class="checkboxes" name="logid[]" value="<?php echo $info['logid']?>" />
                        <span></span>
                    </label></td>
