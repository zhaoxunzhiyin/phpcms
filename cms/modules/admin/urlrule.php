...
class urlrule extends admin {
	...
	function add() {
		if(IS_POST) {
			...
			if ($info['module'] == 'search') {
				$info['file'] = 'search';
			}
			if (!$info['file']) {
				dr_admin_msg(0, L('urlrule_file').L('empty'), array('field' => 'file'));
			}
			if (!$info['example']) {
				dr_admin_msg(0, L('urlrule_example').L('empty'), array('field' => 'example'));
			}
			if (!$info['urlrule']) {
				dr_admin_msg(0, L('urlrule_url').L('empty'), array('field' => 'urlrule'));
			}
			if ($info['module'] != 'content' || ($info['module'] == 'content' && $info['file'] == 'search')) {
				$info['ishtml'] = 0;
			}
			...
			if($this->url_ifok($info['urlrule'], $info['ishtml'])==false){
				dr_admin_msg(0, 'url规则里含有非法php字符', array('field' => 'urlrule'));
			}
			$this->db->insert($info);
			$this->public_cache_urlrule();
	...
	function edit() {
		if(IS_POST) {
			...
			$this->public_cache_urlrule();
			dr_admin_msg(1, L('update_success'));
		}
		$show_validator = $show_header = true;
	...
	public function help() {
		...
		include $this->admin_tpl('urlrule_help');
	}
	// 伪静态
				...
				if (strstr($urlrule, '|')) {
					...
					if ($r['file']=='search' && $urlrule[1]) {
						...
						if (!$preg || !$rname) {
							...
							if (isset($rname['{catdir}'])) {
								// 目录格式
								$rule = 'index.php?m=content&c=search&a=init&catdir=$'.$rname['{catdir}'].'&rewrite=$'.($rname['{param}'] ? $rname['{param}'] : $rname['{rewrite}']);
							} elseif (isset($rname['{categorydir}'])) {
								// 层次目录格式
								$rule = 'index.php?m=content&c=search&a=init&catdir=$'.$rname['{categorydir}'].'&rewrite=$'.($rname['{param}'] ? $rname['{param}'] : $rname['{rewrite}']);
							} elseif (isset($rname['{parentdir}'])) {
								// 层次目录格式
								$rule = 'index.php?m=content&c=search&a=init&catdir=$'.$rname['{parentdir}'].'&rewrite=$'.($rname['{param}'] ? $rname['{param}'] : $rname['{rewrite}']);
							} else {
								// catid模式
								$rule = 'index.php?m=content&c=search&a=init&catid=$'.$rname['{catid}'].'&rewrite=$'.($rname['{param}'] ? $rname['{param}'] : $rname['{rewrite}']);
							}
							if (isset($write[$preg])) {
								$error.= "<p>".$cname."与".$write[$preg]."规则存在冲突</p>";
							} else {
					...
					if ($r['file']=='search' && $urlrule[0]) {
						...
						if (!$preg || !$rname) {
							...
							if (isset($rname['{catdir}'])) {
								// 目录格式
								$rule = 'index.php?m=content&c=search&a=init&catdir=$'.$rname['{catdir}'];
							} elseif (isset($rname['{categorydir}'])) {
								// 层次目录格式
								$rule = 'index.php?m=content&c=search&a=init&catdir=$'.$rname['{categorydir}'];
							} elseif (isset($rname['{parentdir}'])) {
								// 层次目录格式
								$rule = 'index.php?m=content&c=search&a=init&catdir=$'.$rname['{parentdir}'];
							} else {
								// catid模式
								$rule = 'index.php?m=content&c=search&a=init&catid=$'.$rname['{catid}'];
							}
							if (isset($write[$preg])) {
								$error.= "<p>".$cname."与".$write[$preg]."规则存在冲突</p>";
							} else {
					...
					if ($r['file']=='search') {
						if (strstr($urlrule, '{param}') || strstr($urlrule, '{rewrite}')) {
							$rule = $urlrule;
							$cname = "搜索页(分页)（{$rule}）";
							list($preg, $rname) = $this->_rule_preg_value($rule);
							if (!$preg || !$rname) {
			...
			if ($r['module']=='search') {
				if (strstr($urlrule, '|')) {
					...
					if ($urlrule[1]) {
						...
						list($preg, $rname) = $this->_rule_preg_value($rule);
						if (!$preg || !$rname) {
							$error.= "<p>".$cname."格式不正确，需要手动写解析规则</p>";
						} elseif (!isset($rname['{param}']) && !isset($rname['{rewrite}'])) {
							$error.= "<p>".$cname."缺少{param}标签，需要手动写解析规则</p>";
						} else {
							$rule = 'index.php?m=search&c=index&a=init&rewrite=$'.($rname['{param}'] ? $rname['{param}'] : $rname['{rewrite}']);
							if (isset($write[$preg])) {
								$error.= "<p>".$cname."与".$write[$preg]."规则存在冲突</p>";
							} else {
								$write[$preg] = $cname;
					...
					if ($urlrule[0]) {
						...
						if (!$preg) {
							...
							$rule = 'index.php?m=search&c=index&a=init';
							if (isset($write[$preg])) {
								$error.= "<p>".$cname."与".$write[$preg]."规则存在冲突</p>";
							} else {
								$write[$preg] = $cname;
