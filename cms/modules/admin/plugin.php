...
class plugin extends admin {
	...
	public function delete() {
		if($this->input->post('dosubmit')) {
			$pluginid = intval($this->input->post('pluginid'));
	...
	public function listorder() {
		if($this->input->post('dosubmit')) {
			if ($this->input->post('listorders') && is_array($this->input->post('listorders'))) {
				foreach($this->input->post('listorders') as $pluginid => $listorder) {
	...
	public function design() {
	    ...
	    if($this->input->post('dosubmit')) {
			$data['identification'] = $this->input->post('info')['identification'];
			$data['realease'] = date('YMd',SYS_TIME);
			$data['dir'] = $this->input->post('info')['identification'];
			$data['appid'] = '';
			$data['plugin'] = array(
							'version' => '0.0.2',
							'name' => $this->input->post('info')['name'],
							'copyright' => $this->input->post('info')['copyright'],
							'description' => "",
	...
	public function appcenter() {
		$data = array();
		$p = intval($this->input->get(p)) ? intval($this->input->get(p)) : 1;
		$s = 8;
		
		$data = file_get_contents('http://open.cms.cn/index.php?m=open&c=api&a=get_applist&s='.$s.'&p='.$p);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		
		$recommed_data = file_get_contents('http://open.cms.cn/index.php?m=open&c=api&a=get_recommed_applist&s=5&p=1');
		$recommed_data = array_iconv(json_decode($recommed_data, true),'utf-8',CHARSET);
		
		$focus_data = file_get_contents('http://open.cms.cn/index.php?m=open&c=api&a=get_app_focus&num=3');
	...
	public function appcenter_detail() {
		...
		$data = file_get_contents('http://open.cms.cn/index.php?m=open&c=api&a=get_detail_byappid&id='.$id);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		extract($data);		
		if($appname) {
...
EOF;
			...
			//静态程序路径
			$copy_to_staticspath = CMS_PATH.'statics'.DIRECTORY_SEPARATOR.'plugin';

			//应用文件夹名称
			$appdirname = $data['appenname'];
	
			$this->copyfailnum = 0;
			$this->copydir($copy_from.DIRECTORY_SEPARATOR.'cms'.DIRECTORY_SEPARATOR.'plugin', $copy_to_pcpath, $this->input->get('cover'));
			$this->copydir($copy_from.DIRECTORY_SEPARATOR.'statics'.DIRECTORY_SEPARATOR.'plugin', $copy_to_staticspath, $this->input->get('cover'));
	...
	public function public_appcenter_ajx_detail() {
		...
		echo $this->input->get('jsoncallback').'(',$data,')';
		exit;		
	}
	...
	public function config() {
		if($this->input->post('dosubmit')) {
			$pluginid = intval($this->input->post('pluginid'));
			foreach ($this->input->post('info') as $_k => $_v) {
			...
			if(!$this->input->get('module')) {	
				...
				$plugin_module = trim($this->input->get('module'));
	...
	public function status() {
		$disable = intval($this->input->get('disable'));
		$pluginid = intval($this->input->get('pluginid'));
