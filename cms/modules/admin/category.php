...
class category extends admin {
	...
	function __construct() {
		...
		$file = CONFIGPATH.'category.php';
		if (is_file($file)) {
			$this->cat_config = require $file;
			if (!$this->cat_config || !is_array($this->cat_config)) {
				$this->cat_config = [
	...
	function config_add() {
		...
		if (IS_POST) {
			...
			pc_base::load_sys_class('config')->file(CONFIGPATH.'category.php', '栏目配置文件', 32)->to_require($cat_config);
	...
	private function repair_data($psize, $mid) {
		...
		foreach ($_data as $t) {
			if($mid == 'cache' && $t['type'] == 0) {
				$this->content_db->set_model($t['modelid']);
