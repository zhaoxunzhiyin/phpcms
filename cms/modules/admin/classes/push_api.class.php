		...
		if ($arr['dosubmit']) {
			...
			if (is_array($arr['posid']) && !empty($arr['posid']) && is_array($param) && !empty($param)) {
				foreach ($arr['posid'] as $pid) {
					...
					foreach ($param as $d) {
						...
						$delete_ids = $pos_data->select(array('posid'=>$pid),'id',(intval($position_info[$pid]['maxnum'])-1).",10000",'listorder DESC');
						if ($delete_ids) {
							foreach ($delete_ids as $k => $v) {
								$pos_data->delete(array('id'=>$v['id'], 'posid'=>$pid));
							}
						}
			...
			if(!empty($positions)) {
				foreach ($positions as $pid => $p) {
					...
					//获取栏目下所有子栏目
					if ($p['catid']) $catids = explode(',',dr_cat_value($p['catid'], 'arrchildid'));
