...
class googlesitemap extends admin {
	private $input,$header,$charset,$footer,$baidunews_footer,$items,$baidunew_items,$siteid,$categorys,$baidunews,$content_db;
	...
	function set() {
		...
		//根据当前站点,取得文件存放路径
 		...
 		if($siteid==1){
 			...
 			$dir = CMS_PATH.$html_root.DIRECTORY_SEPARATOR.dr_site_info('dirname', $siteid).DIRECTORY_SEPARATOR;
 		}
 		//模型缓存
 		$modelcache = getcache('model','commons');
 		
 		//获取当前站点域名,下面生成URL时会用到.
 		$this_domain = substr(dr_site_info('domain', $siteid), 0,strlen(dr_site_info('domain', $siteid))-1);
   		if($dosubmit) {
				//生成百度新闻
					...
					$this->content_db = pc_base::load_model('content_model');
 					foreach ($catids as $catid) {
  					...
  					$baidunews_file = $dir.'baidunews.xml';
   					...
   					@mkdir($dir,0777,true);
 					$this->baidunews_build($baidunews_file,$this_domain,$this->input->post('email'),$this->input->post('time')); 
 			    }
			    
				//生成网站地图
				$content_priority = $this->input->post('content_priority');
				$content_changefreq = $this->input->post('content_changefreq'); 
				$num = $this->input->post('num') ? intval($this->input->post('num')) : 100;
				...
				foreach($new_model as $modelid=>$m) {//每个模块取出num条数据 
					...
					foreach ($result as $arr){
						if(substr($arr['url'],0,1)=='/'){
							$url = new_html_special_chars(clearhtml($domain.$arr['url']));
						}else {
							$url = new_html_special_chars(clearhtml($arr['url']));
						}
