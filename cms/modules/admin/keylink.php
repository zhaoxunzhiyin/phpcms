...
class keylink extends admin {
	...
	function init () {
		...
		$pages = $this->db->pages;	
		$big_menu = array('javascript:artdialog(\'add\',\'?m=admin&c=keylink&a=add\',\''.L('add_keylink').'\',450,200);void(0);', L('add_keylink'));
	...
	function edit() {
		if(IS_POST){
			...
			$info = $this->input->post('info');
			!$info['word'] && dr_json(0, L('input').L('keylink'), array('field' => 'word'));
			if ($this->db->count(array('keylinkid<>'=>$keylinkid, 'name'=>$info['word']))) {
				dr_admin_msg(0, L('keylink').L('exists'), array('field' => 'word'));
			}
			!$info['url'] && dr_json(0, L('input_siteurl'), array('field' => 'url'));
