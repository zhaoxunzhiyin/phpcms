<?php 
	...
	function set_config($config, $filename="system") {
		...
		foreach($config as $k=>$v) {
			if(in_array($k,array('js_path','css_path','img_path','mobile_js_path','mobile_css_path','mobile_img_path','editor','timezone','needcheckcomeurl','admin_log','gzip','errorlog','auth_key','connect_enable', 'upload_url','sina_akey', 'sina_skey', 'snda_enable', 'snda_status', 'snda_akey', 'snda_skey', 'qq_akey', 'qq_skey','qq_appid','qq_appkey','qq_callback','keywordapi','xunfei_aid','xunfei_skey','baidu_aid','baidu_skey','baidu_arcretkey','baidu_qcnum'))) {
				$v = trim($v);
				$configs[$k] = $v;
