...
$authkey = upload_key('1,jpg|jpeg|gif|png|webp,0,1,,,,,,0');
$p = dr_authcode(array(
    'file_upload_limit' => 1,
...
<?php if ($data['type'] == 1) :?>
  <tr>
    <td class="y-bg"><textarea cols="80" id="data" name="data" rows="10" class="dr_ueditor dr_ueditor_content"><?php echo new_html_special_chars($data['data'])?></textarea>
...
<fieldset id="history" style="display:none">
    ...
    <div class="table-list">
        ...
        <td><a href="?m=block&c=block_admin&a=history_restore&id=<?php echo $val['id']?>"><?php echo L('restore')?></a> <a href="javascript:void(0);" onclick="Dialog.confirm('<?php echo L('are_you_sure_you_want_to_delete')?>',function(){redirect('?m=block&c=block_admin&a=history_del&id=<?php echo $val['id']?>&pc_hash='+pc_hash);});"><?php echo L('delete')?></a></td>
        </tr>
        <?php endforeach;?>
</tbody>
</table>
...
<div class="row">
    <div class="col-md-12 col-sm-12 text-right"><?php if($pages):?><?php echo $pages;?><?php endif;?></div>
</div>
</fieldset>
...
function search_content() {
    omnipotent('search_content','?m=block&c=block_admin&a=public_search_content','<?php echo L('search_content')?>',1,600,400)
}
...
function form_submit(id) {
    ...
    if (title == '') {
        Dialog.alert('<?php echo L('title').L('empty')?>');
        $('#title_'+id).focus();
        return false;
    }
    if (url == '') {
        Dialog.alert('<?php echo L('link').L('empty')?>');
        $('#url_'+id).focus();
        return false;
    }
