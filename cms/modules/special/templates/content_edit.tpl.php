...
$authkey = upload_key('1,jpg|jpeg|gif|png|webp,0,1,300,300,,,,0');
$p = dr_authcode(array(
    'file_upload_limit' => 1,
    'file_types_post' => 'jpg|jpeg|gif|png|webp',
    'size' => 0,
    'allowupload' => 1,
    'thumb_width' => 300,
    'thumb_height' => 300,
...
<div class="page-container" style="margin-bottom: 0px !important;">
    <div class="page-content-wrapper">
        <div class="page-content page-content3 mybody-nheader main-content<?php if (param::get('is_iframe') && !param::get('is_menu')) {?> main-content2<?php }?>">
                <div class="page-body">
<form name="myform" id="myform" action="?m=special&c=content&a=edit&specialid=<?php echo $this->input->get('specialid')?>&id=<?php echo $this->input->get('id')?>" class="form-horizontal" method="post">
    <div class="">
        <div class="row ">
        ...
        <label class="form-control-static"><i class="fa fa-bold" onclick="input_font_bold()" style="cursor:pointer"></i></label></div></div></div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-10">
                                    <input type='text' name='info[keywords]' id='keywords' value='<?php echo $info['keywords']?>' style='width:400px' data-role='tagsinput'>
            ...
            <label><span id="paginationtype1" style="display:<?php if($data['paginationtype']==1) {?>block<?php } else {?>none<?php }?>"><label><input name="data[maxcharperpage]" type="text" id="maxcharperpage" value="<?php echo $data['maxcharperpage']?>" size="8" maxlength="8"></label><?php echo L('number_of_characters')?></span></label>
                                </div>
                            </div>
                   </div>
                    </div>
                    ...
                    <div class="portlet-body">
                        <div class="form-body">
                          <div class="form-group">
                                ...
                                <div class="col-md-10">
                                    <div class="upload-pic img-wrap"><div class="bk10"></div><div class="row fileupload-buttonbar" id="fileupload_thumb"><div class="col-lg-12"><input type="hidden" name="info[thumb]" value="<?php echo $info['thumb']?>" id="thumb">
                        <p><a href="javascript:;" onclick="h5upload('<?php echo SELF;?>', 'thumb_images', '<?php echo L('file_upload')?>','thumb','thumb_images','<?php echo $p?>','content','39','<?php echo $authkey?>',<?php echo SYS_EDITOR;?>);return false;"><img src="<?php if($info['thumb']) { echo dr_get_file($info['thumb']); } else {?><?php echo IMG_PATH;?>icon/upload-pic.png<?php }?>" id="thumb_preview" width="135" height="113" style="cursor:hand" /></a><div class="mpreview"><?php if(is_numeric($info['thumb'])) {echo '<a href="javascript:crop_cut_thumb($(\'#thumb\').val());"><i class="fa fa-cut"></i></a>';}?></div></p><input type='hidden' name="crop_thumb" id="crop_thumb" class="hide"><script type="text/javascript">function crop_cut_thumb(id){
    ...
    var diag = new Dialog({id:'crop',title:'<?php echo L('crop_thumb')?>',url:'<?php echo SELF;?>?m=content&c=content&a=public_crop&module=special&spec=2&aid='+id+'&input=thumb&preview=thumb_preview',width:w,height:h,modal:true});diag.onOk = function(){$DW.dosbumit();return false;};diag.onCancel=function() {$DW.close();};diag.show();
};</script> <label><button type="button" onclick="$('#thumb_preview').attr('src','<?php echo IMG_PATH;?>icon/upload-pic.png');$('#thumb').val('');$('#fileupload_thumb').find('.thumb-delete').hide();$('#fileupload_thumb').find('.mpreview').html('');return false;" class="btn red btn-sm thumb-delete"<?php if(!$info['thumb']) { echo ' style="display:none"';}?>> <i class="fa fa-trash"></i> <?php echo L('cancel_thumb')?></button></label></div></div></div>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-10">
                                    <?php echo form::date('info[inputtime]', format::date($info['inputtime'], 1) , 1);?>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-md-2"><?php echo L('template_style')?></label>
                                <div class="col-md-10">
                                    <?php echo form::select($template_list, $data['style'], 'name="data[style]" id="style" onchange="load_file_list(this.value)"', L('please_select'))?>
                                </div>
                            </div>
                            <div class="form-group">
