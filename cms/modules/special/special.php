...
class special extends admin {
	...
	public function add() {
		if (IS_POST) {
			...
			if (!$type[1]['name']) {
				dr_json(0, L('type_name').L('empty'), array('field' => 'type_name_1'));
			}
			if (!$type[1]['typedir']) {
				dr_json(0, L('type_path').L('empty'), array('field' => 'type_path_1'));
			}
			if ($special['ishtml'] && !$special['filename']) {
				dr_json(0, L('special_filename').L('empty'), array('field' => 'filename'));
			}
			...
			dr_json(1, L('add_special_success'));
		} else {
			//获取站点模板信息
			...
			$reply_url = '?m=special&c=special&a=init&menuid='.$this->input->get('menuid').'&pc_hash='.dr_get_csrf_token();
	...
	public function edit() {
		...
		if (IS_POST) {
			...
			$this->special_cache();
			dr_json(1, L('edit_special_success'));
		} else {
	...
	public function import() {
		if(IS_POST) {
			...
			if(!$typeid) dr_admin_msg(0,L('select_type'), HTTP_REFERER);
			foreach($ids as $id) {
				$this->special_api->_import($this->input->post('modelid'), $this->input->get('specialid'), $id, $typeid, $this->input->post('listorder')[$id]);
			}
			$html = pc_base::load_app_class('html', 'special'); 
	...
	private function check($data, $a = 'add') {
		...
		if(!$data['banner']) dr_admin_msg(0,L('banner_no_empty'), array('field' => 'banner'));
		if(!$data['thumb']) dr_admin_msg(0,L('thumb_no_empty'), array('field' => 'thumb'));
