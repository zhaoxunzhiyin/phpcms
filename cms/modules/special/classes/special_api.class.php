...
class special_api {
	...
	private $input, $db, $type_db, $c_db, $data_db, $attachment_db, $search_db;
	public $pages;
	...
	public function _get_type($specialid = 0) {
		...
		return pc_base::load_sys_class('form')::select($arr, '', 'name="typeid", id="typeid"', L('please_select'));
	}
	...
	public function _del_special($id = 0) {
		...
		$info['filename'] = str_replace('..','',(string)$info['filename']);
		if ($info['siteid']>1) {
			if ($info['ishtml']) {
				...
				dr_dir_delete(SYS_HTML_ROOT.'/'.$site_info['dirname'].'/special/'.$info['filename'], TRUE);
			}
		} else {
			if ($info['ishtml']) {
				dr_dir_delete(CMS_PATH.SYS_HTML_ROOT.DIRECTORY_SEPARATOR.'special'.DIRECTORY_SEPARATOR.$info['filename'], TRUE); //删除专题目录
			}
		}
			...
			if ($siteid>1) {
				...
				for ($i = 1; $i>0; $i++) {
					...
					if (!file_exists(CMS_PATH.$file)) {
						...
						unlink(CMS_PATH.$file);	//删除生成的静态文件
					}
				}
			} else {
				for ($i = 1; $i>0; $i++) {
				...
				if ($siteid>1) {
					...
					for ($i = 1; $i>0; $i++) {
						...
						if (!file_exists(CMS_PATH.$file[1])) {
							...
							unlink(CMS_PATH.$file[1]);	//删除生成的静态文件
						}
					}
				} else {
					for ($i = 1; $i>0; $i++) {
