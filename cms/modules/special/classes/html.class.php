...
class html {
	private $input, $db, $type_db, $c_db, $data_db, $site, $queue;
	...
	public function _create_content($contentid = 0) {
		...
		if ($arr_content['paginationtype']) {			//文章使用分页时
			if($arr_content['paginationtype']==1) {
				if (strpos((string)$arr_content['content'], '[/page]')!==false) {
		...
		$CONTENT_POS = strpos((string)$arr_content['content'], '[page]');
		if ($CONTENT_POS !== false) {
			$contents = array_filter(explode('[page]', $arr_content['content']));
			$pagenumber = count($contents);
			$END_POS = strpos((string)$arr_content['content'], '[/page]');
