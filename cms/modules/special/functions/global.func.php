...
function content_url($contentid = 0, $page = 1, $addtime = '', $type = 'html', $site_info = '') {
	...
	switch ($type) {
		case 'html':
			if ($site_info['dirname']) {
				...
				if ($page==1) {
					...
					$showurl[0] = $showurl[1] = SYS_HTML_ROOT.'/special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'.html';
					$showurl[0] = $app_path.$showurl[0];
				} else {
					$url[0] = $url[1] = SYS_HTML_ROOT.'/special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'-'.$page.'.html';
					$url[0] = $app_path.$url[0];
					$showurl[0] = $showurl[1] = SYS_HTML_ROOT.'/special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'-{page}.html';
					$showurl[0] = $app_path.$showurl[0];
				}
			}
			break;
		...
		case 'php':
			if ($page==1) {
				$url[0] = $url[1] = APP_PATH.'index.php?m=special&c=index&a=show&id='.$contentid;
				$showurl[0] = $showurl[1] = APP_PATH.'index.php?m=special&c=index&a=show&id='.$contentid;
			} else {
				$url[0] = $url[1] = APP_PATH.'index.php?m=special&c=index&a=show&id='.$contentid.'&page='.$page;
				$showurl[0] = $showurl[1] = APP_PATH.'index.php?m=special&c=index&a=show&id='.$contentid.'&page={page}';
			}
			break;
	}
...
function get_pic_content($pics) {
	...
	$category_setting = dr_string2array($category['setting']);
	$siteid = $CAT['siteid'];
