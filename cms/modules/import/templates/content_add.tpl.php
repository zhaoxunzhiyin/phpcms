...
<fieldset>
	<legend><?php echo L('import_type_content')?></legend>
    ...
    <td> 
    <?php echo form::radio(array('gbk'=>'GBK', 'gb2312'=>'gb2312', 'utf8mb4'=>'UTF8MB4', 'utf-8'=>'UTF8','latin1'=>'latin1'),$setting['dbcharset'],'name="setting[dbcharset]" id="dbcharset"',L('please_select'));?>
	</td>
	</tr>
...
<fieldset>
	...
	<table width="100%" class="table_form">
     <tbody> 
 	<tr>
		<th width="150"><?php echo L('defaultcatid')?> :</th>
		<td>
		
		
		<select name="setting[defaultcatid]" id="setting[defaultcatid]">
		<option value=""><?php echo L('please_select')?></option>
		<?php foreach($arr_cat as $s=>$g){?>
		<option value="<?php echo $s;?>" <?php if($setting['defaultcatid']==$s){echo 'selected style="color:red"';}?>><?php echo $g;?></option>
		<?php }?>
	...
	?>
	<tr>
		<th width="150"><?php echo $g;?> :</th>
		<td><input type="text" name="setting[catids][<?php echo $k;?>]" size="15" class="input_blur" <?php if(isset($setting['catids'][$k])){?>value="<?=$setting['catids'][$k]?>" <?php } ?>> <?php echo L('old_catid_info')?></td>
	</tr>
	<?php }?>
	 </tbody>
	 </table>
