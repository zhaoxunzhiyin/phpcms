...
function dr_rewrite_routes_path($uri) {
	$uri = trim($uri, '/');
	$path = CACHE_PATH . 'route/'.(defined('SITE_ID') ? SITE_ID : 1).'/'; // 路由缓存根目录
...
function dr_save_rewrite_routes($uri, $routes) {
	...
	$uri = str_replace(['{$page}', '[page]', '%7Bpage%7D', '%5Bpage%5D', '%7bpage%7d', '%5bpage%5d', '%7B%24page%7D', '%5B%24page%5D', '%7b%24page%7d', '%5b%24page%5d'], '{page}', $uri);
	$routes = str_replace(['{$page}', '[page]', '%7Bpage%7D', '%5Bpage%5D', '%7bpage%7d', '%5bpage%5d', '%7B%24page%7D', '%5B%24page%5D', '%7b%24page%7d', '%5b%24page%5d'], '{page}', $routes);
	list($save, $hash) = dr_rewrite_routes_path($uri);
	if (!is_dir($save)) {
		mkdir($save, 0755, true);
	}
...
if (is_cli()) {
	...
	if (!IS_ADMIN && CMSURI && !IS_API) {
		...
		if ($rcache) {
			...
			// 自定义URL解析规则
			$routes = [];
			$routes[str_replace('/', '\/', ltrim(WEB_PATH, '/')).'index\.html(.*)'] = 'index.php?m=content&c=index';
			$routes[str_replace('/', '\/', ltrim(WEB_PATH, '/')).'404\.html(.*)'] = 'index.php?m=404&uri='.CMSURI;
			$routes[str_replace('/', '\/', ltrim(WEB_PATH, '/')).'rewrite-test.html(.*)'] = 'index.php?m=content&c=index&a=test';
