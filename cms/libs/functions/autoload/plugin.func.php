...
function plugin_install_stat($appid){
	...
	$url = 'http://open.cms.cn/api.php?op=appstatus&isinstall=1';
...
function app_validity_check($appid) {
	...
	$appid = intval($appid);
	$url = $header = '';
	$url = 'http://open.cms.cn/api.php?op=appstatus';
	$headers = get_headers($url.'&appid='.$appid,1);
	$status = $headers['pc_appstatus'];
	return $status;
}
...
function plugin_url($data = '',$type = '') {
	...
	$args = '';
	if($data == '') {
		$args = $input->get('args') ? '-'.$input->get('args') : '';		
...
*/
function getargs() {
	$string = base64_decode($input->get('args'));
	parse_str($string,$g);
	return $g;
}
?>