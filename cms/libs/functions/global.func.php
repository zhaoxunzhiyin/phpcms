				...
				if (is_file($path.'classes/'.$dir.'_auth'.$endfix.'.class.php')) {
					...
					foreach ($_clink as $k => $v) {
						...
						if (strpos($v['name'], '_') === 0
							...
							$_clink[$k]['name'] = call_user_func(array($obj, substr($v['name'], 1)), $dir);
						}
						// check权限验证
						if (isset($v['check']) && $v['check'] && method_exists($obj, $v['check'])
							&& !call_user_func(array($obj, $v['check']), $dir, [])) {
							unset($_clink[$k]);
							continue;
						}
					}
					// 权限验证
					if ($pos == 'link' && method_exists($obj, 'is_link_auth') && $obj->is_link_auth($dir)) {
						$data = array_merge($data, $_clink);
					} elseif ($pos == 'bottom' && method_exists($obj, 'is_bottom_auth') && $obj->is_bottom_auth($dir)) {
						$data = array_merge($data, $_clink);
					} else {
