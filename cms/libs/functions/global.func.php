...
if (!function_exists('dr_show_hits')) {
	function dr_show_hits($id, $modelid = 0, $dom = '') {
		$is = $dom;
		...
		return $html.'<script type="text/javascript">$.ajax({ type: "GET", url:"'.APP_PATH.'api.php?op=count&id='.$id.'&modelid='.$modelid.'", dataType: "jsonp", success: function(data){ if (data.code) { '.$rt.' } else { dr_tips(0, data.msg); } } }); </script>';
	}
}
/**
 * 栏目下级或者同级栏目
