...
// 获取内容的搜索词
function dr_get_content_kws($value, $siteid = '', $catid = '', $mobile = 0) {
	if (is_array($value)) {
		return $value;
	} elseif (!$value) {
		return [];
	...
	foreach ($tag as $t) {
		$t = trim($t);
		if ($t) {
			$rt[$t] = $catid ? dr_module_search_url($catid, [], 'keyword', $t, $mobile) : dr_search_url([], 'keyword', $t, $siteid, $mobile);
		}
	}
...
// 模型搜索url组合
function dr_module_search_url($catid, $params = [], $name = '', $value = '', $mobile = 0) {
	$str_catid = (is_array($name) ? !dr_in_array('catid', $name) : $name != 'catid') || $name == 'catid' && $value == '' ? '&catid='.$catid : '';
	...
	!$siteid && $siteid = get_siteid();
	$row = $model_arr[(int)dr_cat_value($catid, 'modelid')];
	unset($model_arr);

	if (!$row) {
		return '模型[' . dr_cat_value($catid, 'modelid') . ']缓存不存在';
	}
	$form_cache = $sitemodel[(string)$row['tablename']];
	...
	$str_url = $mobile ? (dr_site_info('mobile_domain', $siteid) ? dr_site_info('mobile_domain', $siteid) : MOBILE_PATH) : dr_site_info('domain', $siteid);
	$php_url = 'index.php?'.trim('m=content&c=search&a=init'.$str_catid.'&' . (is_array($params) ? http_build_query($params) : ''), '&');
	$not_diy_router = isset($form_cache['setting']['not_diy_url_route']) ? $form_cache['setting']['not_diy_url_route'] : 0;
	$rule = explode('|', (string)$urlrules[(int)$form_cache['setting']['search']['urlrule']]);
	if ($rule && $rule[0]) {
		$data['categorydir'] = get_categorydir($catid);
		$data['parentdir'] = get_parentdir($catid);
		$data['catdir'] = dr_cat_value($catid, 'catdir');
		$data['catid'] = $catid;
		$data['param'] = dr_search_rewrite_encode($params, $form_cache['setting']['search']);
		if ($params && !$data['param']) {
			log_message('debug', '模型['.$row['tablename'].']无法通过[搜索参数字符串规则]获得参数');
		}
		$url = ltrim($data['param'] && $rule[1] ? $rule[1] : $rule[0], '/');
		$my_url = get_url_value($data, $url);
		!$not_diy_router && dr_save_rewrite_routes($my_url, trim($php_url, '/'));
		return $str_url.$my_url;
	} else {
		return $str_url.$php_url;
	}
}
function get_categorydir($catid, $dir = '') {
	$setting = string2array(dr_cat_value($catid, 'setting'));
	if ($setting['create_to_html_root']) return $dir;
	if (dr_cat_value($catid, 'parentid')) {
		$r = dr_cat_value(dr_cat_value($catid, 'parentid'));
		$dir = $r['catdir'].'/'.$dir;
		return get_categorydir(dr_cat_value($catid, 'parentid'), $dir);
	} else {
		return $dir;
	}
}
function get_parentdir($catid) {
	return dr_cat_value($catid, 'parentdir').dr_cat_value($catid, 'catdir');
}
// 搜索url组合
function dr_search_url($params = [], $name = '', $value = '', $siteid = '', $mobile = 0) {
	!$siteid && $siteid = get_siteid();
	...
	$str_url = $mobile ? (dr_site_info('mobile_domain', $siteid) ? dr_site_info('mobile_domain', $siteid) : MOBILE_PATH) : dr_site_info('domain', $siteid);
	$php_url = 'index.php?'.trim('m=search&c=index&a=init&' . (is_array($params) ? http_build_query($params) : ''), '&');
	$not_diy_router = isset($setting['not_diy_url_route']) ? $setting['not_diy_url_route'] : 0;
	$rule = explode('|', (string)$urlrules[(int)$setting['urlrule']]);
	if ($rule && $rule[0]) {
		$data['param'] = dr_search_rewrite_encode($params, $setting);
		if ($params && !$data['param']) {
			log_message('debug', '全站搜索无法通过[搜索参数字符串规则]获得参数');
		}
		$url = ltrim($data['param'] && $rule[1] ? $rule[1] : $rule[0], '/');
		$my_url = get_url_value($data, $url);
		!$not_diy_router && dr_save_rewrite_routes($my_url, trim($php_url, '/'));
...
// 伪静态替换
function get_url_value($data, $rule, $prefix = '', $is_ltrim = 1) {
	pc_base::load_sys_class('php5replace', '', 0);
	$rep = new \php5replace($data);
	$url = (string)$rep->replace($rule);
	if (dr_is_url($url)) {
		return $url; // 带域名的url直接返回
	}
	$is_ltrim ? $url = ltrim(str_replace('//', '/', $url), '/') : $url = str_replace('//', '/', $url);
	return $prefix.$url;
}
/**
 * 生成Tag URL
...
function tag_url($keyword, $siteid = '', $catid = '', $modelid = -1, $mobile = 0){
	!$siteid && $siteid = get_siteid();
	if ($modelid >= 0) {
		return dr_search_url($modelid ? ['typeid'=>$modelid] : [], 'keyword', $keyword, $siteid, $mobile);
	}
	if ($catid) {
		return dr_module_search_url($catid, [], 'keyword', $keyword, $mobile);
	}
	return ($mobile ? (dr_site_info('mobile_domain', $siteid) ? dr_site_info('mobile_domain', $siteid) : MOBILE_PATH) : dr_site_info('domain', $siteid)).'index.php?m=content&c=tag&a=lists&tag='.urlencode($keyword).'&siteid='.$siteid;
}
...
function show_url($url, $catid = '', $id = '', $is_client = IS_CLIENT) {
	...
	if ($is_client) {
		if (!dr_is_url($url) && $content_ishtml) {
 ...
 * 提取关键字
 */
function dr_get_keyword_data($title, $content, $length = SYS_BAIDU_QCNUM) {
	if (!$title) {
	...
	if (SYS_KEYWORDAPI == 1) {
		...
		$rt = $phpanalysis->GetFinallyKeywords((int)$length);
		if (!$rt) {
 ...
 * 提取关键字
 */
function dr_get_keywords($title, $content = '', $length = SYS_BAIDU_QCNUM) {
	if (!$title) {
		return '';
	}
	...
	$rt = dr_get_keyword_data($title, $content, $length);
	if (!$rt['code']) {
		return '';
	}
	return $rt['msg'];
...
function dr_array2array($a1, $a2) {
	return array_merge(
		is_array($a1) ? $a1 : array(),
		is_array($a2) ? $a2 : array()
	);
}

/**
 * 两数组覆盖合并，$a1是老数据，$a2是新数据
 */
...
if (!function_exists('dr_is_safe_function')) {
	function dr_is_safe_function($func) {
		$dangerous = [
			...
			'escapeshellarg', 'escapeshellcmd', 'eval', 'exec', 'fp', 'fput',
			...
			'posix_uname', 'proc_close', 'proc_get_status', 'proc_nice', 'proc_open',
			'proc_terminate', 'shell_exec', 'syslog', 'system', 'xmlrpc_entity_decode',
		];
		if (in_array(strtolower((string)$func), $dangerous, true)) {
			return false;
		}
		return true;
	}
...
function dr_file_preview_html($value, $target = 0) {
	...
	if (dr_is_image($ext)) {
		...
		$value = dr_file($value);
		$url = $target ? $value.'" target="_blank' : 'javascript:dr_preview_audio(\''.$value.'\');';
		return '<a href="'.$url.'"><img src="'.$file.'"></a>';
	} else {
...
function dr_file_list_preview_html($t) {
	...
	} elseif ($t['fileext'] == 'mp3') {
		return '<a href="javascript:dr_preview_audio(\''.dr_get_file_url($t).'\');"><img src="'.$file.'"></a>';
	} else {
...
function sys_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0) {
	...
	if($result && $operation == 'DECODE') {
		if((substr($result, 0, 10) == 0 || (dr_is_numeric(substr($result, 0, 10)) && substr($result, 0, 10) - SYS_TIME > 0)) && substr($result, 10, 16) == substr(md5(substr($result, 26).$keyb), 0, 16)) {
	...
	if(file_exists(TPLPATH.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		...
		if(!file_exists($compiledtplfile) || (file_exists(TPLPATH.'default/'.pc_base::load_sys_class('service')->get_dir().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(TPLPATH.'default/'.pc_base::load_sys_class('service')->get_dir().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
			...
			if (CI_DEBUG) {
				...
				if ($template == 'msg' || $template == '404' || $module == '404') {
					return admin_template('api_msg', 'admin');
				}
...
function dr_member_info($userid, $name = '', $cache = -1) {
	...
	if (!$data) {
		...
		if(is_array($member_modelinfo)) {
			$data = dr_array2array($data, $member_modelinfo);
		}
		$member_db->set_model();
		if ($data) {
...
if (!function_exists('dr_show_hits')) {
	function dr_show_hits($id, $modelid = 0, $module = '', $dom = '') {
		$is = $dom;
		...
		return $html.'<script type="text/javascript">$.ajax({ type: "GET", url:"'.WEB_PATH.'api.php?op=count&id='.$id.($modelid ? '&modelid='.$modelid : '').($module ? '&module='.$module : '').'", dataType: "jsonp", success: function(data){ if (data.code) { '.$rt.' } else { dr_tips(0, data.msg); } } }); </script>';
	}
}
/**
 * 栏目下级或者同级栏目
...
function dr_go($catid, $id, $allurl = 0, $mobile = 0) {
	$id = intval($id);
	$modelid = intval(dr_cat_value($catid, 'modelid'));
	...
	if (!empty($allurl)) {
		if (strpos($r['url'], '://')===false) {
			if ($mobile) {
				$r['url'] = sitemobileurl($category[$catid]['siteid']).$r['url'];
			} else {
				$r['url'] = siteurl($category[$catid]['siteid']).$r['url'];
			}
		}
	}
	return $r['url'];
...
function sql_module($tablename, $sql, $ismain = 0) {
	if ($tablename && $sql) {
		...
		if (!$ismain) {
			...
			for ($i = 0;; $i ++) {
				if (!$content_db->table_exists(str_replace('_data_0', '_data_'.$i, $tablename))) {
					break;
				}
...
function string2img($txt, $fonttype = 5, $fontsize = 16, $font = '', $fontcolor = 'FF0000',$transparent = '1') {
	...
	if(function_exists("imagepng")) {
		...
		$txt = '<img src="'.WEB_PATH.'api.php?op=creatimg&txt='.$txt.'&fonttype='.$fonttype.'&fontsize='.$fontsize.'&font='.$font.'&fontcolor='.$fontcolor.'&transparent='.$transparent.'" align="absmiddle">';
	}
	return $txt;
}
...
function qrcode($text, $thumb = '', $background = '', $pcolor = '', $mcolor = '', $level = 'H', $size = 5) {
	...
	return WEB_PATH.'api.php?op=qrcode&thumb='.urlencode($thumb).'&text='.urlencode($text).'&background='.$background.'&pcolor='.$pcolor.'&mcolor='.$mcolor.'&size='.$size.'&level='.$level;
}
