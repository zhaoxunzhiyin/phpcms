...
function dr_get_content_kws($value, $siteid = '', $catid = '', $modelid = -1, $mobile = 0) {
	...
	if (intval($mobile)) {
		...
		foreach ($tag as $t) {
			$t = trim($t);
			if ($t) {
				$rt[$t] = $modelid >= 0 ? mobile_tag_url($t, $siteid, $catid, $modelid) : ($catid ? mobile_tag_url($t, $siteid, $catid) : mobile_tag_url($t, $siteid));
			}
		}
	} else {
		...
		foreach ($tag as $t) {
			$t = trim($t);
			if ($t) {
				$rt[$t] = $modelid >= 0 ? tag_url($t, $siteid, $catid, $modelid) : ($catid ? tag_url($t, $siteid, $catid) : tag_url($t, $siteid));
			}
		}
	}
...
function dr_url_jump($content) {
	...
	foreach ($domain_arr as $item) {
		...
		if ($isreplace == true) {
			$html = dr_str_replace_once($item[1], WEB_PATH.'index.php?m=404&c=index&a=jump&go=' . md5($item[1]) . '" target="_blank" rel="nofllow" src="' . $item[1], $item[0]);
...
function template($module = 'content', $template = 'index', $style = '') {
	...
	if(file_exists(TPLPATH.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		...
		if(!file_exists($compiledtplfile) || (file_exists(TPLPATH.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(TPLPATH.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
			$template_cache->template_compile($module, $template, 'default');
		} elseif (!file_exists(TPLPATH.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
			if (IS_DEV) {
				log_message('error', '模板文件['.TPLPATH.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html]不存在');
			}
			if (IS_MOBILE) {
				if ($module=='mobile') {
 ...
 * 模块的clink值
 */
function module_clink($type = '', $data = []) {
	...
	// 加载模块自身的
	if (ROUTE_M && is_file(PC_PATH.'modules/'.ROUTE_M.'/config/clink'.$endfix.'.php')) {
		$_clink = require PC_PATH.'modules/'.ROUTE_M.'/config/clink'.$endfix.'.php';
		if ($_clink) {
			if (is_file(PC_PATH.'modules/'.ROUTE_M.'/classes/auth'.$endfix.'.class.php')) {
				$obj = pc_base::load_sys_class('auth'.$endfix, ROUTE_M);
				foreach ($_clink as $k => $v) {
					// 动态名称
					if (strpos($v['name'], '_') === 0 && method_exists($obj, substr($v['name'], 1))) {
						$_clink[$k]['name'] = call_user_func(array($obj, substr($v['name'], 1)), ROUTE_M);
					}
					// 对象存储
					$_clink[$k]['model'] = $obj;
				}
				// 权限验证
				if (method_exists($obj, 'is_link_auth') && $obj->is_link_auth(ROUTE_M)) {
					$data = $_clink;
				}
			} else {
				$data = $_clink;
	...
	foreach ($local as $dir => $path) {
		// 排除模块自身
		if (strtolower($dir) == ROUTE_M) {
			continue;
		}
		// 判断模块目录
		if (is_file($path.'config/clink'.$endfix.'.php')) {
			$_clink = require $path.'config/clink'.$endfix.'.php';
			if ($_clink) {
				if (is_file($path.'classes/auth'.$endfix.'.class.php')) {
					$obj = pc_base::load_sys_class('auth'.$endfix, $dir);
					foreach ($_clink as $k => $v) {
						// 动态名称
						if (strpos($v['name'], '_') === 0 && method_exists($obj, substr($v['name'], 1))) {
							$_clink[$k]['name'] = call_user_func(array($obj, substr($v['name'], 1)), ROUTE_M);
					...
					$data = array_merge($data , $_clink) ;
					CI_DEBUG && log_message('debug', '配置文件（'.$path.'Config/Clink'.$endfix.'.php'.'）没有定义权限验证类（'.$path.'Classes/Auth'.$endfix.'.class.php'.'）');
				}
			}
		}
	}
	...
	if ($data) {
		foreach ($data as $i => $t) {
			...
			if (!$t['url']) {
				unset($data[$i]); // 没有url
				CI_DEBUG && log_message('debug', 'Clink（'.$t['name'].'）没有设置url参数');
				continue;
			}
 ...
 * 模块的cbottom值
 */
function module_cbottom($type = '', $data = []) {
	...
	// 加载模块自身的
	if (ROUTE_M && is_file(PC_PATH.'modules/'.ROUTE_M.'/config/cbottom'.$endfix.'.php')) {
		$_clink = require PC_PATH.'modules/'.ROUTE_M.'/config/cbottom'.$endfix.'.php';
		if ($_clink) {
			if (is_file(PC_PATH.'modules/'.ROUTE_M.'/classes/auth'.$endfix.'.class.php')) {
				$obj = pc_base::load_sys_class('auth'.$endfix, ROUTE_M);
				if (method_exists($obj, 'is_bottom_auth') && $obj->is_bottom_auth(ROUTE_M)) {
					$data = array_merge($data , $_clink);
				}
			} else {
				$data = array_merge($data , $_clink);
	...
	foreach ($local as $dir => $path) {
		...
		// 判断模块目录
		if (module_exists($dir) && is_file($path.'config/cbottom'.$endfix.'.php')) {
			// 表示模块非模块
			$_clink = require $path.'config/cbottom'.$endfix.'.php';
			if ($_clink) {
				if (is_file($path.'classes/auth'.$endfix.'.class.php')) {
					$obj = pc_base::load_app_class('auth'.$endfix, $dir);
					if (method_exists($obj, 'is_bottom_auth') && $obj->is_bottom_auth(ROUTE_M)) {
						$data = array_merge($data , $_clink);
					}
				} else {
...
function dr_catpos($catid, $symbol = ' > ', $url = true, $html = '') {
	...
	$siteids = getcache('category_content','commons');
	$siteid = $siteids[$catid];
	$cat = getcache('cache', 'module/category-'.$siteid.'-data');
	if (!isset($cat[$catid])) {
		return '';
	}
...
// 获取主栏目
function get_category_main($siteid = '') {
	if (!$siteid) $siteid = get_siteid();
...
function subcat($parentid = NULL, $type = NULL,$self = '0', $siteid = '') {
	...
	$category = getcache('cache', 'module/category-'.$siteid.'-data');
...
function check_captcha($id) {
	...
	if (!$code) {
		IS_DEV && log_message('error', '图片验证码未生成（'.md5($input->ip_address().$input->get_user_agent()).'）');
		return false;
