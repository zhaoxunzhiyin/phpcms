...
function dr_get_content_kws($value, $siteid = '', $catid = '', $mobile = 0) {
	...
	foreach ($tag as $t) {
		$t = trim($t);
		if ($t) {
			$rt[$t] = $catid ? dr_module_search_url($catid, [], 'keyword', $t, $mobile) : dr_search_url([], 'keyword', $t);
		}
	}
...
function dr_module_search_url($catid, $params = [], $name = '', $value = '', $mobile = 0) {
	...
	$str_url = ($mobile || is_mobile($siteid) ? (dr_site_info('mobile_domain', $siteid) ? dr_site_info('mobile_domain', $siteid) : MOBILE_PATH) : dr_site_info('domain', $siteid));
	$php_url = 'index.php?'.trim('m=content&c=search&a=init'.((is_array($name) ? !dr_in_array('catid', $name) : $name != 'catid') || $name == 'catid' && $value == '' ? '&catid='.$catid : '').'&' . (is_array($params) ? http_build_query($params) : ''), '&');
	$rule = $urlrules[(int)$form_cache['setting']['search']['urlrule']];
	$category_dir = get_categorydir($catid);
	$parentdir = get_parentdir($catid);
	$rule = str_replace(array('{$categorydir}', '{$parentdir}', '{$catdir}', '{$catid}'),array($category_dir, $parentdir, dr_cat_value($catid, 'catdir'), $catid), (string)$rule);
	$rule = explode('|',(string)$rule);
	if ($rule && $rule[0]) {
...
function dr_search_url($params = [], $name = '', $value = '', $siteid = '', $mobile = 0) {
	...
	$rule = $urlrules[(int)$setting['urlrule']];
