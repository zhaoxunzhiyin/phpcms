...
function dr_module_search_url($catid, $params = [], $name = '', $value = '', $mobile = 0) {
	...
	!$siteid && $siteid = get_siteid();
	$row = $model_arr[dr_cat_value($catid, 'modelid')];
	unset($model_arr);

	if (!$row) {
		return '模型[' . dr_cat_value($catid, 'modelid') . ']缓存不存在';
	}
	$form_cache = $sitemodel[$row['tablename']];
	...
	if ($rule && $rule[0]) {
		...
		$url = ltrim($data['param'] ? $rule[1] : $rule[0], '/');
...
function tag_url($keyword, $siteid = '', $catid = '', $modelid = -1, $mobile = 0){
	!$siteid && $siteid = get_siteid();
	if ($modelid >= 0) {
		return dr_search_url([], 'keyword', $keyword, $siteid, $mobile);
	}
	if ($catid) {
