...
function module_clink($type = '', $data = []) {
	return module_click('link', $type, $data);
}
...
function module_cbottom($type = '', $data = []) {
	return module_click('bottom', $type, $data);
}

function module_click($pos, $type = '', $data = []) {
				...
				if (is_file($path.'classes/'.$dir.'_auth'.$endfix.'.class.php')) {
					...
					foreach ($_clink as $k => $v) {
						...
						if (isset($v['check']) && $v['check'] && method_exists($obj, $v['check'])
							&& !call_user_func(array($obj, $v['check']), ROUTE_M, [])) {
							unset($_clink[$k]);
							continue;
						}
					}
					// 权限验证
					if ($pos == 'link' && method_exists($obj, 'is_link_auth') && $obj->is_link_auth(ROUTE_M)) {
						$data = array_merge($data, $_clink);
					} elseif ($pos == 'bottom' && method_exists($obj, 'is_bottom_auth') && $obj->is_bottom_auth(ROUTE_M)) {
						$data = array_merge($data, $_clink);
					} else {
