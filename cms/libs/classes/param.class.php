...
class param {
	...
	public function route_a() {
		...
		$a = $this->safe_deal($a);
		if (IS_ADMIN) {
			if (!self::get_session('userid') && $a!='public_logout' && $a!='public_login_screenlock') {
				$a = SYS_ADMIN_PATH;
			}
		}
		if (!IS_ADMIN) {
	...
	public static function set_cookie($name, $value = '', $expire = 0) {
		$expire = $expire > 0 ? SYS_TIME + $expire : ($value == '' ? SYS_TIME - 3600 : 0);
		...
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($name.'['.$k.']', $v, $expire, COOKIE_PATH, COOKIE_DOMAIN, ($s ? true : false), true);
			}
		} else {
			setcookie($name, $value, $expire, COOKIE_PATH, COOKIE_DOMAIN, ($s ? true : false), true);
		}
	}
	...
	public static function get_cookie($name, $default = false) {
		...
		if(in_array($name,array('_userid','userid','siteid','_groupid','_roleid'))) {
			$value = intval($value);
		} elseif(in_array($name,array('_username','username','_nickname','admin_username','sys_lang'))) {
	...
	public static function set_session($key, $value = '') {
		pc_base::load_sys_class('session')->set(SYS_KEY.$key, $value);
	}

	public static function get_session($key) {
		return pc_base::load_sys_class('session')->get(SYS_KEY.$key);
	}

	public static function del_session($key) {
		return pc_base::load_sys_class('session')->remove(SYS_KEY.$key);
	}

	public static function setTempdata($key, $value, $time) {
		pc_base::load_sys_class('session')->setTempdata(SYS_KEY . $key, $value, $time);
	}

	public static function getTempdata($key = null) {
		return pc_base::load_sys_class('session')->getTempdata(SYS_KEY . $key);
	}
