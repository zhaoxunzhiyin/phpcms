...
class model {
	...
	public function is_data_table($table, $tid) {
		if ($tid > 0 && !$this->table_exists($table.$tid)) {
			...
			if ($database['default']['type']!='sqlite3') {
				...
				$fields = $this->get_fields($table.'0');
				$sql = 'CREATE TABLE IF NOT EXISTS `'.$table.$tid.'` ('.PHP_EOL;
				if (isset($fields) && is_array($fields) && $fields) {
					$key_t = -1;
					$count_t = dr_count($fields);
					foreach ($fields as $k=>$t) {
						$key_t++;
						$is_first = $key_t == 0 ? 1 : 0;
						$is_last = $count_t == $key_t+1 ? 1 : 0;
						$sql .= '`'.$k.'` '.strtolower($t['type']).($t['notnull'] ? ' NOT NULL' : '').($t['dflt_value'] != '' ? ' DEFAULT '.(is_numeric($t['dflt_value']) ? $t['dflt_value'] : '\''.$t['dflt_value'].'\'') : '').($t['pk'] ? ' PRIMARY KEY AUTOINCREMENT' : '').($is_last ? PHP_EOL : ','.PHP_EOL);
					}
				}
				$sql .= ')';
				$this->query($sql);
	...
	// 字段值是否存在
	public function is_exists($id, $name, $value, $where = array()) {
		$key = $this->get_primary();
		$a = array($key[0].'<>'=>$id, $name=>$value);
