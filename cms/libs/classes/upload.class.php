...
class upload {
    ...
    public function save_data($data, $related = '') {
        ...
        $uploadedfile['isimage'] = dr_is_image($data['ext']) ? 1 : 0;
    ...
    public function clear_data($index) {
        ...
        // 删除缓存
        pc_base::load_sys_class('cache')->del_file('attach-info-'.$index['aid'], 'attach');
    ...
    protected function _rand_save_file_name($file) {
        ...
        if (function_exists('openssl_random_pseudo_bytes')) {
            $crypto = false;
            $b = openssl_random_pseudo_bytes(16, $crypto);
            if ($b !== false) {
                return bin2hex($b);
            }
        }
        // 末位兜底（极旧环境）：熵低于 random_bytes，仅作兼容；长度与上面一致为 32 位十六进制
        return substr(
            hash('sha256', (string)SYS_TIME.(is_array($file) ? dr_array2string($file) : $file).mt_rand()),
            0,
            32
        );
    }
        ...
        if (isset($config['save_name']) && $config['save_name']) {
            if ($config['save_name'] == 'null') {
                ...
                if (is_array($file) && isset($file['name']) && $file['name']) {
                    $name = trim(pc_base::load_sys_class('pinyin')->result(dr_safe_filename($file['name'])), '.'.$file_ext);
                }
            } else {
