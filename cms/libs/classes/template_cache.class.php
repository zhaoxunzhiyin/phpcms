...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default/pc') {
		...
		if (str_replace('/mobile', '/pc', $style) != 'default' && !file_exists ( $tplfile )) {
			$style = 'default/'.(pc_base::load_sys_class('service')->is_mobile() ? 'mobile' : 'pc');
			$tplfile = TPLPATH.'default/'.(pc_base::load_sys_class('service')->is_mobile() ? 'mobile' : 'pc').DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
		}
