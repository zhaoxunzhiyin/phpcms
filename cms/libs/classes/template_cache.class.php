...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default/pc') {
		...
		if (!file_exists($tplfile)) {
			if (CI_DEBUG) {
				...
				if ($template == 'msg' || $template == '404' || $module == '404') {
					pc_base::load_sys_class('service')->admin_display('api_msg', 'admin');
					exit;
				}
		...
		if ($strlen = file_put_contents($compiledtplfile, $content, LOCK_EX) === FALSE) {
			pc_base::load_sys_class('service')->show_error('模板缓存文件 ('.$compiledtplfile.') 创建失败，请将caches目录权限设为777');
		}
		...
		if (in_array($op,$tools)) {
			switch ($op) {
				...
				case 'table':
						...
						foreach ($params as $t) {
							list($var, $val) = self::get_params_var_val($t);
							if (!$var) {
								continue;
							}
	...
	// 解析参数变量和值
	private static function get_params_var_val($t) {
		if (!$t) {
			return ['', ''];
		}
		...
		foreach ($arr as $a => $v) {
			if (strpos($t, $a) !== FALSE) {
				...
				$val = trim(trim(substr($t, strpos($t, $a) + strlen($a)), '"'), '\'');
				return [$v.$var, $val];
			}
		}
		return ['', ''];
