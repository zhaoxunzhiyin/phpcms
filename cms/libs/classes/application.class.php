...
class application {
	...
	public function __construct() {
		...
		// 站点共享变量
		define('SITE_NAME', dr_site_info('name', SITE_ID));
		define('SITE_URL', dr_site_info('domain', SITE_ID));
		define('SITE_MURL', dr_site_info('mobile_domain', SITE_ID));
		$this->userid = $this->param->get_cookie('_userid');
		// 访客唯一标识
		!defined('USER_HTTP_CODE') && define('USER_HTTP_CODE', md5($this->userid.pc_base::load_sys_class('input')->ip_address().pc_base::load_sys_class('input')->get_user_agent()));
		// 是否会员
		...
		if ($this->userid) {
			$this->member = dr_member_info($this->userid);
		} else {
			$this->member = [];
		}
	...
	private function init() {
		...
		if (IS_API === 'api') {
			if($this->userid) {
				if(pc_base::load_model('member_model')->get_one(array('userid'=>$this->userid),'islock')['islock']) dr_msg(0, L('user_is_lock', '', 'member'));
			}
	...
	public function verify() {
		...
		$token = dr_safe_replace(pc_base::load_sys_class('input')->post(csrf_token()));
		$hash = csrf_hash();
		if (!isset($token, $hash) || !hash_equals($hash, $token)) {
			CI_DEBUG && log_message('debug', 'CSRF跨站验证拦截（系统码'.$hash.' / 提交码'.$token.'）');
			dr_exit_msg(0, 'CSRF跨站验证超时请重试', '', array('name' => csrf_token(), 'value' => $hash));
		}
