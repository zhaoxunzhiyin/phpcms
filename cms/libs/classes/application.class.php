	...
	private function init() {
		...
		if (IS_API === 'api') {
			if(intval(pc_base::load_sys_class('param')::get_cookie('_userid'))) {
				if(pc_base::load_model('member_model')->get_one(array('userid'=>intval(pc_base::load_sys_class('param')::get_cookie('_userid'))),'islock')['islock']) exit('错误的请求！');
			}
			$op = pc_base::load_sys_class('input')->get('op') && trim(pc_base::load_sys_class('input')->get('op')) ? trim(pc_base::load_sys_class('input')->get('op')) : exit('操作不能为空');
