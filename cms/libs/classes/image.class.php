...
class image {
    ...
    public function thumb($img, $width = 0, $height = 0, $water = 0, $mode = 'auto', $webimg = 0) {
        ...
        if (is_numeric($img)) {
            ...
            $attach['filepath'] = dr_date(SYS_TIME, 'Y/md/').$attach['id'].'.'.$attach['fileext'];
        }

        // 图片缩略图文件
        if ($attach['remote'] && $attach['url']) {
            list($cache_path, $cache_url) = array(SYS_THUMB_PATH, SYS_THUMB_URL);
            $cache_file = md5($attach['aid']).'/'.$width.'x'.$height.($water ? '_water' : '').(dr_is_empty($mode) ? '' : '_'.$mode).'.'.$attach['fileext'];
            // 创建缓存目录
