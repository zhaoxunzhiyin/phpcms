...
class image {
    ...
    public function initialize($props = array())
    {
        ...
        {
            $this->set_error('您的服务器必须支持GD映像库才能确定映像属性。');
            return FALSE;
        }
    ...
    public function image_process_imagemagick($action = 'resize')
    {
        ...
        {
            $this->set_error('图像库的路径不正确。请在您的图像首选项中设置正确的路径。');
            return FALSE;
        }
        ...
        if ($retval > 0)
        {
            $this->set_error('图像处理失败。请验证您的服务器是否支持所选的协议，以及映像库的路径是否正确。');
            return FALSE;
        }
        chmod($this->full_dst_path, $this->file_permissions);
        return TRUE;
    ...
    public function image_create_gd($path = '', $image_type = '')
    {
        ...
        switch ($image_type)
        {
            case 1:
                ...
                {
                    $this->set_error(array('您的服务器不支持处理此类图像所需的GD功能。', '由于许可限制，GIF图像通常不受支持。您可能不得不使用JPG或PNG图像。'));
                    return FALSE;
                }
            ...
            case 2:
                ...
                {
                    $this->set_error(array('您的服务器不支持处理此类图像所需的GD功能。', '不支持JPG图像。'));
                    return FALSE;
                }
            ...
            case 3:
                ...
                {
                    $this->set_error(array('您的服务器不支持处理此类图像所需的GD功能。', '不支持PNG图像。'));
                    return FALSE;
                }
            ...
            case 18:
                ...
                {
                    $this->set_error(array('您的服务器不支持处理此类图像所需的GD功能。', '不支持WEBP图像。'));
                    return FALSE;
                }
                return imagecreatefromwebp($path);
            default:
                $this->set_error(array('您的服务器不支持处理此类图像所需的GD功能。'));
                return FALSE;
        }
    }
    // --------------------------------------------------------------------
    ...
    public function image_save_gd($resource)
    {
        ...
        {
            case 1:
                if ( ! $this->save_gd_image($resource, 'imagegif')) {
                    return FALSE;
                }
                break;
            case 2:
                if ( ! $this->save_gd_image($resource, 'imagejpeg', TRUE)) {
                    return FALSE;
                }
                break;
            case 3:
                if ( ! $this->save_gd_image($resource, 'imagepng')) {
                    return FALSE;
                }
                break;
            case 18:
                if ( ! $this->save_gd_image($resource, 'imagewebp', TRUE)) {
                    return FALSE;
                }
                break;
            case 19:
    ...
    protected function save_gd_image($resource, $function_name, $with_quality = FALSE) {

        if ( ! function_exists($function_name)) {
            $this->set_error($function_name.'函数不存在');
            return FALSE;
        }

        $unlink_msg = '';
        if (file_exists($this->full_dst_path) && !is_writable($this->full_dst_path)) {
            $unlink_msg = @unlink($this->full_dst_path) ? '已删除不可写旧文件' : '删除不可写旧文件失败';
        }
        ...
        $status = $with_quality
            ? @$function_name($resource, $this->full_dst_path, $this->quality)
            : @$function_name($resource, $this->full_dst_path);

        if ($status) {
            return TRUE;
        }

        $dst_dir = dirname($this->full_dst_path);
        $detail = [
            '路径: '.$this->full_dst_path,
            '目录存在: '.(is_dir($dst_dir) ? '是' : '否'),
            '目录可写: '.(is_dir($dst_dir) && is_writable($dst_dir) ? '是' : '否'),
            '文件可写: '.(!file_exists($this->full_dst_path) || is_writable($this->full_dst_path) ? '是' : '否'),
        ];

        if ($with_quality) {
            $detail[] = '质量: '.$this->quality;
        }
        if ($unlink_msg) {
            $detail[] = $unlink_msg;
        }
        ...
        if ($last_error && isset($last_error['message']) && $last_error['message']) {
            $detail[] = 'PHP错误: '.$last_error['message'];
        }

        $this->set_error(CI_DEBUG ? $function_name.'图片储存失败（'.implode('，', $detail).'）' : '图片储存失败');
        return FALSE;
    }
    // --------------------------------------------------------------------
    /**
    ...
    public function image_display_gd($resource)
    {
        ...
        header('Last-Modified: '.gmdate('D, d M Y H:i:s', SYS_TIME).' GMT');
        switch ($this->image_type)
        {
    ...
    public function get_image_properties($path = '', $return = FALSE)
    {
        ...
        if ( ! file_exists($path))
        {
            $this->set_error('图像的路径不正确。');
            return FALSE;
        }
        $vals = getimagesize($path);
        if ($vals === FALSE)
        {
            $this->set_error('提供的图像无效。');
            return FALSE;
        }
    ...
    public function thumb($img, $width = 0, $height = 0, $water = 0, $mode = 'auto', $webimg = 0) {
        ...
        if (is_numeric($img)) {
            ...
            } elseif (!in_array($attach['fileext'], array('png', 'jpeg', 'jpg', 'webp', 'avif'))) {
                CI_DEBUG && log_message('debug', '图片[id#'.$img.']扩展名不符合条件，thumb函数无法调用');
                return $attach['url'] ? $attach['url'] : IMG_PATH.'nopic.gif';
            }
        } else {
            $attach = [
                'aid' => md5($img),
            ...
            $attach['filepath'] = dr_date(SYS_TIME, 'Y/md/').$attach['aid'].'.'.$attach['fileext'];
        }

        // 图片缩略图文件
        list($cache_path, $cache_url) = array(SYS_THUMB_PATH, SYS_THUMB_URL);
        $cache_file = md5($img).'/'.$width.'x'.$height.($water ? '_water' : '').(dr_is_empty($mode) ? '' : '_'.$mode).'.'.$attach['fileext'];
        ...
        if ($attach['remote'] && $attach['url']) {
            if ($webimg) {
                ...
                if (!$data) {
                    CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']的URL['.$attach['url'].']无法获取远程附件数据，thumb函数无法调用');
                    return $attach['url'];
                }
                dr_mkdirs(CACHE_PATH.'caches_attach/caches_data/');
                $file = CACHE_PATH.'caches_attach/caches_data/'.$attach['aid'].'.'.$attach['fileext'];
                if (!file_put_contents($file, $data)) {
                    CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']的URL['.$attach['url'].']无法写入附件缓存目录，thumb函数无法调用');
                    return $attach['url'];
                }
            } else {
                // 远程图片进行带规则的缩略图处理
                $remote = get_cache('attachment', $attach['remote']);
                if ($remote) {
                    if (is_dir(SYS_UPLOAD_PATH.$remote['value']['path'])) {
                        // 相对路径
                        $file = SYS_UPLOAD_PATH.$remote['value']['path'].$attach['filepath'];
                    } else if (is_dir(CMS_PATH.$remote['value']['path'])) {
                        // 相对路径
                        $file = CMS_PATH.$remote['value']['path'].$attach['filepath'];
                    } else {
                        $file = $remote['value']['path'].$attach['filepath'];
                    }
                    if (!is_file($file)) {
                        // 文件不存在表示网络地址
                        ...
                        //输出直接地址
                        CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']自定义策略地址，将原样输出');
                        return $attach['url'];
                    }
                } else {
                    //输出直接地址
                    CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']自定义策略失效，将原样输出');
                    return $attach['url'];
                }
            }
        } elseif (!is_file($file)) {
            // 本地图片不存在
            CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']的文件['.$attach['file'].']无法写入附件缓存目录，thumb函数无法调用');
            return IMG_PATH.'nopic.gif';
        }

        if ($width == 0 && $height == 0 && $water == 0) {
            CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']参数为空，将原样输出');
            return $attach['url']; // 原样输出
        }

        $this->image_info = getimagesize($file);
        if ($this->memory_limit($this->image_info)) {
            CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']的URL['.$attach['url'].']分辨率太大导致服务器内存溢出，无法进行缩略图处理，已按原图显示');
            return $attach['url']; // 原样输出
        }
        ...
        if (!is_file($cache_path.$cache_file)) {
            CI_DEBUG && log_message('debug', '图片[id#'.$attach['aid'].']的URL['.$attach['url'].']生成失败'.((dr_count($this->error_msg) > 0) ? '('.implode(' - ', $this->error_msg).')' : '').'['.$cache_file.']原样输出');
            return $attach['url']; // 原样输出
        }

        // 水印处理
        if ($water) {
            $data = get_cache('site', (defined('SITEID') && SITEID ? SITEID : (defined('SITE_ID') && SITE_ID!=1 ? SITE_ID : get_siteid())), 'param');
            if ($data) {
        ...
        // 钩子处理
        $rs = pc_base::load_sys_class('hooks')::trigger_callback('thumb_save', $cache_path, $cache_file);
    ...
    protected function imageCropper_size($target_width, $target_height) {
        ...
        return array(floor($cropped_width), floor($cropped_height));
    }
    ...
    // 解析画笔笔迹
    public function parse_brush_points($value) {
        ...
        $points = [];
        if (isset($value['brush_points']) && $value['brush_points']) {
            $json = urldecode((string)$value['brush_points']);
        ...
        $legacy = [];
        if (isset($value['brush']) && $value['brush']) {
            if (is_string($value['brush'])) {
                $legacy = json_decode($value['brush'], true);
            } elseif (is_array($value['brush'])) {
                $legacy = $value['brush'];
            }
        }

        $base_color = $value['brush_color'] ?? ($legacy['color'] ?? '#ff0000');
        $color = $this->parse_hex_color($base_color);
        if (!$points && isset($legacy['points']) && is_array($legacy['points'])) {
        ...
        foreach ($points as $v) {
            ...
            $x = max(0, (int)($v['x'] ?? 0));
            $y = max(0, (int)($v['y'] ?? 0));
            $r = max(4, min(60, (int)($v['r'] ?? 12)));
            $point_color = $this->parse_hex_color($v['c'] ?? '', $color);
            $result_points[] = [
                'x' => $x,
                'y' => $y,
                'r' => $r,
    ...
    // 解析颜色
    public function parse_hex_color($value, $default = [255, 0, 0]) {

        if (!is_string($value) || !preg_match('/^#?([a-f0-9]{6})$/i', trim($value), $match)) {
            return $default;
        }
    ...
    // 裁剪后偏移画笔坐标
    public function offset_brush_points(array $points, $crop_x, $crop_y, $crop_w, $crop_h) {

        if (!$points) {
            return [];
        }

        $crop_w = max(0, (int)$crop_w);
        $crop_h = max(0, (int)$crop_h);
        $result = [];
        foreach ($points as $point) {
            if (!isset($point['x'], $point['y'])) {
                continue;
            }
            $x = (int)$point['x'] - (int)$crop_x;
            $y = (int)$point['y'] - (int)$crop_y;
            if ($crop_w && ($x < 0 || $x >= $crop_w)) {
                continue;
            }
            if ($crop_h && ($y < 0 || $y >= $crop_h)) {
                continue;
            }
            $point['x'] = max(0, $x);
            $point['y'] = max(0, $y);
    ...
    // 图片画笔处理（画笔轨迹）
    public function image_brush($file, array $brush) {

        $points = $brush['points'];

        if (!$points || !is_file($file)) {
            return false;
        }
        ...
        foreach ($points as $point) {
            $px = max(0, min($width - 1, (int)$point['x']));
            $py = max(0, min($height - 1, (int)$point['y']));
            $radius = max(4, min(60, (int)($point['r'] ?? 12)));
            $color_arr = isset($point['c']) && is_array($point['c']) ? $point['c'] : $brush['color'];
            $draw_color = imagecolorallocate($image, (int)$color_arr[0], (int)$color_arr[1], (int)$color_arr[2]);
            imagefilledellipse($image, $px, $py, $radius * 2, $radius * 2, $draw_color);
        }
        ...
        if (function_exists('exif_imagetype')) {
            ...
            $type = $img_info && isset($img_info[2]) ? (int)$img_info[2] : 0;
        }
        $saved = false;
        switch ($type) {
            case IMAGETYPE_GIF:
                $saved = imagegif($image, $file);
                break;
            case IMAGETYPE_PNG:
                ...
                break;
            case IMAGETYPE_WEBP:
                $saved = function_exists('imagewebp') ? imagewebp($image, $file, 90) : false;
                break;
            case IMAGETYPE_JPEG:
            default:
                $saved = imagejpeg($image, $file, 90);
                break;
        }
