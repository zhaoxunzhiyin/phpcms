...
final class db_mysqli {
	...
	public function query($sql, $replace = false) {
		strpos($sql, 'DROP TABLE') !== false && $this->execute('SET FOREIGN_KEY_CHECKS=0;');
		$query = $this->execute($sql);
		strpos($sql, 'DROP TABLE') !== false && $this->execute('SET FOREIGN_KEY_CHECKS=1;');
		return $query;
	}
	...
	public function get_fields(string $table) {
		...
		$this->execute("SHOW FULL COLUMNS FROM $table");
		while($r = $this->fetch_next()) {
			$this->dataCache['field_names'][$table][$r['Field']] = $r;
		}
		return $this->dataCache['field_names'][$table];
	}
