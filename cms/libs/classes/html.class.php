...
class html {
        ...
        foreach ($cat as $i => $t) {
            ...
            if ($t['modelid'] == 0 && $t['type'] == 1) {
                ...
                if ($t['child']) {
                    ...
                    if(file_exists(TPLPATH.$template_list.DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR.$template.'.html')) {
                        $html = file_get_contents(TPLPATH.$template_list.DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR.$template.'.html');
                    }
    ...
    public function get_show_data($modelid, $param) {
        ...
        if ($param['catids']) {
            ...
            if ($catids) {
                $cats = getcache('cache', 'module/category-'.$param['siteid'].'-data');
