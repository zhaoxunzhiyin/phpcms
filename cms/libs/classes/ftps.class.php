...
class ftps {
	...
	//错误代码
	private $err_code = 0;
	//传送模式{文本模式:FTP_ASCII, 二进制模式:FTP_BINARY} ascii:FTP_ASCII binary:FTP_BINARY
	public $mode = 'binary';
	...
	public function put($remote, $local) {
		...
		if (@ftp_put($this->link, $remote, $local, ($this->mode === 'ascii') ? FTP_ASCII : FTP_BINARY)) {
			return true;
		} else {
			$this->err_code = 7;
			return false;
