...
class service {
    ...
    private $_admin_filename; // 后台主模板名称
    ...
    public function display($module = 'content', $template = 'index', $style = '') {
        ...
        $template && $this->_filename = $module.'/'.$template.'.html';
    ...
    public function admin_display($file, $m = '') {
        ...
        extract($this->_options, EXTR_SKIP);
        $file && $this->_admin_filename = $m.'/'.$file.'.html';
    ...
    public function load_view_file($file) {
        ...
        if (!is_file($file)) {
            $this->show_error('模板缓存文件 ('.$file.') 创建失败，请将caches目录权限设为777');
        }
    ...
    // 后台主模板名称
    public function get_admin_filename() {
        return $this->_admin_filename;
    }
    ...
    public function show_error($msg, $file = '', $fixfile = '') {
        ...
        if (CI_DEBUG || defined('IS_HTML') && IS_HTML) {
            ...
            if (defined('IS_HTML') && IS_HTML) {
                if (pc_base::load_sys_class('input')->get('iframe')) {
                    html_msg(0, $this->_options['my_web_url'].'：'.$msg);
                } else {
