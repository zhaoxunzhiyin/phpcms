...
final class db_sqlite3 {
	...
	public function conn(bool $persistent = false) {
		...
		try {
			...
			$sqlite = ($this->config['password'] === null || $this->config['password'] === '')
	...
	public function connect() {
		...
		if (is_int($this->synchronous)) {
			if (! in_array($this->synchronous, [0, 1, 2, 3], true)) {
				throw new \Error('Invalid synchronous value.');
			}
			$this->link->exec('PRAGMA synchronous = ' . $this->synchronous);
		}
	}
	 ...
	 */
	public function query($sql, $replace = true) {
		$sql = str_ireplace('TRUNCATE', 'DELETE FROM', $sql);
		$sql = str_ireplace('CURDATE()', '\''.dr_date(SYS_TIME, 'Y-m-d').'\'', $sql);
		$sql = str_ireplace('unsigned', '', $sql);
		if ($replace) {
			$sql = preg_replace('/\bCOMMENT\s*\'[^\']*\'/', '', $sql);
			$sql = preg_replace('/AUTO_INCREMENT=\d+/', '', $sql);
			$sql = preg_replace('/AUTO_INCREMENT *= *\d+/', '', $sql);
			$sql = preg_replace('/\bNOT NULL\s*\(\)/i', '', $sql);
			$sql = str_ireplace('NOT NULL PRIMARY KEY AUTOINCREMENT', '<!--cmsnotnullprimarykeyautoincrement-->', $sql);
			$sql = str_ireplace('NOT NULL', '', $sql);
			$sql = str_ireplace('<!--cmsnotnullprimarykeyautoincrement-->', 'NOT NULL PRIMARY KEY AUTOINCREMENT', $sql);
			$sql = preg_replace('/\bAUTO_INCREMENT\b/i', 'NOT NULL PRIMARY KEY AUTOINCREMENT', $sql);
			$sql = preg_replace('/\bDEFAULT CURRENT_TIMESTAMP\b/i', '', $sql);
			$sql = preg_replace('/\bENGINE=\w+\s*/i', '', $sql);
			$sql = preg_replace('/\bDEFAULT CHARSET=\w+\s*/i', '', $sql);
			$sql = preg_replace('/\bCOLLATE=\w+\s*/i', '', $sql);
			$sql = preg_replace('/\bCOLLATE \w+\s*/i', '', $sql);
			$sql = preg_replace('/,\s*PRIMARY KEY\s*\([^)]+\)/i', '', $sql);
			$sql = preg_replace('/=\d+\s*$/i', '', $sql);
			$sql = preg_replace('/,\s*KEY\s+\w+\s*\([^)]+\)/i', '', $sql);
			$sql = preg_replace('/,\s*CONSTRAINT\s+\w+\s+FOREIGN KEY\s*\([^)]+\)\s+REFERENCES\s+\w+\s*\([^)]+\)/i', '', $sql);
			...
			$sql = preg_replace('/numeric\((\d+)\)/i', 'integer ', $sql);
			$sql = preg_replace('/\blongblob\b/i', 'blob', $sql);
			$sql = preg_replace('/\bvarbinary\b/i', 'blob', $sql);
			$sql = preg_replace('/\bbinary\b/i', 'blob', $sql);
			$sql = preg_replace('/\btinyblob\b/i', 'blob', $sql);
			$sql = preg_replace('/\bmediumblob\b/i', 'blob', $sql);
			$sql = preg_replace('/\bfloat\b/i', 'real', $sql);
			$sql = preg_replace('/\bdouble\b/i', 'real', $sql);
			$sql = preg_replace('/\bdecimal\b/i', 'real', $sql);
			...
			$sql = str_ireplace('`datetime`', '`<!--cmsdatetime-->`', $sql);
			$sql = str_ireplace('`date`', '`<!--cmsdate-->`', $sql);
			$sql = str_ireplace('`time`', '`<!--cmstime-->`', $sql);
			$sql = str_ireplace('`year`', '`<!--cmsyear-->`', $sql);
			$sql = preg_replace('/\bdatetime\b/i', 'text', $sql);
			$sql = preg_replace('/\bdate\b/i', 'text', $sql);
			$sql = preg_replace('/\btime\b/i', 'text', $sql);
			$sql = preg_replace('/\byear\b/i', 'text', $sql);
			$sql = str_ireplace('`<!--cmsdatetime-->`', '`datetime`', $sql);
			$sql = str_ireplace('`<!--cmsdate-->`', '`date`', $sql);
			$sql = str_ireplace('`<!--cmstime-->`', '`time`', $sql);
			$sql = str_ireplace('`<!--cmsyear-->`', '`year`', $sql);
			$sql = preg_replace('/\bbit\b/i', 'text', $sql);
			$sql = preg_replace('/\benum\b/i', 'text', $sql);
			$sql = preg_replace('/\bgeometry\b/i', 'text', $sql);
			$sql = preg_replace('/\bgeometrycollection\b/i', 'text', $sql);
			$sql = preg_replace('/\bjson\b/i', 'text', $sql);
			$sql = preg_replace('/\blinestring\b/i', 'text', $sql);
			$sql = preg_replace('/\bmultilinestring\b/i', 'text', $sql);
			$sql = preg_replace('/\bmultipoint\b/i', 'text', $sql);
			$sql = preg_replace('/\bmultipolygon\b/i', 'text', $sql);
			$sql = preg_replace('/\bpoint\b/i', 'text', $sql);
			$sql = preg_replace('/\bpolygon\b/i', 'text', $sql);
			$sql = preg_replace('/\bset\b/i', 'text', $sql);
			$sql = preg_replace('/\bvector\b/i', 'text', $sql);
		}
		strpos($sql, 'DROP TABLE') !== false && $this->execute('PRAGMA foreign_keys = OFF;');
		$query = $this->execute($sql);
		strpos($sql, 'DROP TABLE') !== false && $this->execute('PRAGMA foreign_keys = ON;');
		return $query;
	}
	...
	public function get_fields(string $table) {
		...
		while($r = $this->fetch_next()) {
			$r['Type'] = $r['type'];
			$this->dataCache['field_names'][$table][$r['name']] = $r;
		}
		return $this->dataCache['field_names'][$table];
	}
