        ...
        _renderTemplate:function (data) {
            ...
            for (var i = 0, tmpList; tmpList = data[i++];) {
                ...
                for (var j = 0, tmpObj; tmpObj = tmpList[j++];) {
                    s.push('<label for="radio-' + i + '-' + j + '" class="mt-radio mt-radio-outline">');
                    s.push('<input type="radio" id="radio-' + i + '-' + j + '" name="musicId" class="m-l" onclick="music.doselect(' + (me.pageSize * (i-1) + (j-1)) + ')"/><span></span>');
                    s.push('<var class="m-t">' + me._getMaxText(tmpObj.title) + '</var>');
                    s.push('<var class="m-s">' + me._getMaxText(tmpObj.author) + '</var>');
                    s.push('<var class="m-z">' + me._getMaxText(tmpObj.album_title) + '</var>');
                    s.push('<var class="m-try" onclick="music.doselect(' + (me.pageSize * (i-1) + (j-1)) + ');music.listenTest(this)"></var>');
                    s.push('</label>');
                }
                s.push('</div>');
                s.push('</div>');
