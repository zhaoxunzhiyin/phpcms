...
// 删除文件
function dr_file_delete(url, obj, id, not_sx) {
	if (typeof pc_hash == 'string') url += (url.indexOf('?') > -1 ? '&': '?') + 'pc_hash=' + pc_hash;
	...
	Dialog.confirm('确定要删除本文件吗？', function() {
		$(obj).parents('.files_row').find('.checkboxes').prop("checked", true);
		$.post(url, $("#myform").serialize(), function(data) {
			...
			if (data.code) {
				var id = $(obj).parents('.files_row').children("a").children("img").attr("id");
				var src = $(obj).parents('.files_row').children("a").children("img").attr("path");
				var filename = $(obj).parents('.files_row').children("a").children("img").attr("filename");
				var size = $(obj).parents('.files_row').children("a").children("img").attr("size");
				$(obj).parents('.files_row').removeClass("on");
				$(obj).parents('.files_row').children("a").removeClass("on");
				var length = $(obj).parents('.files_row').children("a[class='on']").children("img").length;
				var ids = strs = filenames = '';
				for(var i=0;i<length;i++){
					ids += '|'+$(obj).parents('.files_row').children("a[class='on']").children("img").eq(i).attr('id');
					strs += '|'+$(obj).parents('.files_row').children("a[class='on']").children("img").eq(i).attr('path');
					filenames += '|'+$(obj).parents('.files_row').children("a[class='on']").children("img").eq(i).attr('filename');
				}
...
function dr_bfb(title, myform, url) {
	...
	layer.open({
		...
		content: url+(myform?'&'+$('#'+myform).serialize():''),
