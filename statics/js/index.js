var calendar = (function (exports) {
  ...
  var calendar = {
     ...
     * @return JSON object
     * @eg:console.log(calendar.newdate(1987,11,01));
     */
    newdate: function newdate(yPara, mPara, dPara) {
      ...
      d = objDate.getDate();
      var hour = objDate.getHours(); //当前系统时间的小时值
      var minute = objDate.getMinutes(); //当前系统时间的分钟值
      var second = objDate.getSeconds(); //当前系统时间的秒钟值
      ...
      var solarDate = this.datefilter(y) + '\u5E74' + this.datefilter(m) + '\u6708' + this.datefilter(d) + '\u65E5';
      ...
      $('.lock-screen-time').html(this.datefilter(hour) + ':' + this.datefilter(minute) + ':' + this.datefilter(second));
      $('#lock-screen-date').html(timevalue + gzY + this.getAnimal(year) + '\u5E74 ' + solarDate);
      $('#lock-screen-week').html('\u3010\u519C\u5386'+ (isLeap ? '\u95F0' : '') + this.toChinaMonth(month) + this.toChinaDay(day) +'\u3011\u661F\u671F' + cWeek);
      setTimeout('calendar.default.newdate()',1000);
    },
    /**
     * 传入阳历年月日获得详细的公历、农历object信息 <=>JSON
     * !important! 公历参数区间1900.1.31~2100.12.31
