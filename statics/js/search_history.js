$(document).ready(function() {
	...
	if(search_history!=null && search_history!='') {
		...
		var exists = in_array(keyword+'|'+typeid, search_s);
		//不存在
		if(exists==-1) {
			if(search_s.length > 5) {
				search_history = search_history.replace(search_s[0]+',', "");
			}
			search_history += ','+keyword+'|'+typeid;
		}
		...
		for(i=0;i<search_s.length;i++) {
			...
			history_html += '<li><a href="?m=search&c=index&a=init&typeid='+keywordtypeid+'&keyword='+keyword+'">'+keyword+'</a></li>';
		}
