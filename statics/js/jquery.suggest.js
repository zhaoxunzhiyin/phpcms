...
//此处针对上面介绍的修改增加的函数
					...
					switch(e.keyCode) {
						...
						case 13: //	enter
							...
							if ($currentResult) {
								...
								window.location.href='?m=search&c=index&a=init&keyword='+$currentResult.text();
							}
			...
			function suggest() {
				...
				if (keyword.length >= options.minchars) {
					$.getScript(options.source+'&keyword='+keyword, function(){
					});
					$results.hide();
					//var items = parseTxt(txt, keyword);
					//displayItems(items);
					$results.show();
