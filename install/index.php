		...
		if(file_exists(CMS_PATH.'install/main/'.$dbfile)) {
			...
			if ($page) {
				...
				if (isset($rows[$key]) && $rows[$key]) {
					...
					_sql_execute($mysqli, "INSERT INTO ".$data['tablepre']."admin (`userid`,`username`,`password`,`roleid`,`encrypt`,`lastloginip`,`lastlogintime`,`email`,`realname`) VALUES ('1','$username','$password','[\"1\"]','$encrypt','','','$email','创始人')");
