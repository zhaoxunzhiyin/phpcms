...
switch($step) {
	...
	case 'sql': //安装数据
		...
		if($version > '4.1' && $data['dbcharset']) {
			mysqli_query($mysqli, 'SET NAMES '.$data['dbcharset']);
		}
		...
		if(file_exists(CMS_PATH.'install/main/'.$dbfile)) {
			...
			if ($page) {
				...
				if (isset($rows[$key]) && $rows[$key]) {
					...
					_sql_execute($mysqli, "INSERT INTO ".$data['tablepre']."admin (`userid`,`username`,`password`,`roleid`,`encrypt`,`lastloginip`,`lastlogintime`,`email`,`phone`,`realname`) VALUES ('1','$username','$password','[\"1\"]','$encrypt','','','$email','','创始人')");
	...
	case 'dbtest':
		...
		if ($data['adminpath']) {
			...
			if(pc_base::load_config('system','admin_login_path')) {
				//建立自定义后台登录目录
				create_folder(CMS_PATH.$data['adminpath']);
