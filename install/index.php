...
switch($step) {
	...
	case 'sql': //安装数据
		...
		$dbfile = 'cms_db.sql';
		$db = pc_base::load_sys_class('db_factory')::get_instance(pc_base::load_config('database'))->get_database('default');
		if(file_exists(CMS_PATH.'install/main/'.$dbfile)) {
			...
			if ($page) {
				...
				if (isset($rows[$key]) && $rows[$key]) {
					...
					$db->query("INSERT INTO ".$data['tablepre']."admin (`userid`,`username`,`password`,`roleid`,`encrypt`,`lastloginip`,`email`,`phone`,`realname`,`lang`) VALUES ('1','$username','$password','[\"1\"]','$encrypt','','$email','','创始人','zh-cn')");
